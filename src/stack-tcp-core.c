...
static void vLOGtcb(const struct TCP_Control_Block *tcb, int dir,
  ...
  snprintf(sz, sizeof(sz), "[%s:%u %4u,%4u] %s:%5u [%4u,%4u] {%s} ",
           fmt1.string, tcb->port_them, tcb->seqno_them - tcb->seqno_them_first,
           tcb->ackno_me - tcb->seqno_them_first, (dir > 0) ? "-->" : "<--",
           tcb->port_me, tcb->seqno_me - tcb->seqno_me_first,
...
static void LOGtcb(const struct TCP_Control_Block *tcb, int dir,
                   const char *fmt, ...) {
  va_list marker;
...
void tcpcon_timeouts(struct TCP_ConnectionTable *tcpcon, unsigned secs,
  ...
  for (;;) {
     ...
     */
    x = stack_incoming_tcp(tcpcon, tcb, TCP_WHAT_TIMEOUT, 0, 0, secs, usecs,
...
void tcpcon_set_parameter(struct TCP_ConnectionTable *tcpcon, const char *name,
  ...
  if (name_equals(name, "hello-string")) {
    ...
    x->hello_length =
        base64_decode((char *)x->hello, value_length, value, value_length);
...
tcpcon_create_table(size_t entry_count, struct stack_t *stack,
                    struct TemplatePacket *pkt_template,
                    OUTPUT_REPORT_BANNER report_banner, struct Output *out,
                    unsigned connection_timeout, uint64_t entropy) {
...
static int TCB_EQUALS(const struct TCP_Control_Block *lhs,
...
/***************************************************************************
 ***************************************************************************/
static void _tcb_change_state_to(struct TCP_Control_Block *tcb,
                                 unsigned new_state) {
...
/***************************************************************************
 ***************************************************************************/
static unsigned tcb_hash(ipaddress ip_me, unsigned port_me, ipaddress ip_them,
  ...
  if (ip_me.version == 6) {
    ...
    index = (unsigned)syn_cookie_ipv4(
 ...
 ***************************************************************************/
struct TCP_Control_Block *
tcpcon_create_tcb(struct TCP_ConnectionTable *tcpcon, ipaddress ip_me,
                  ipaddress ip_them, unsigned port_me, unsigned port_them,
                  unsigned seqno_me, unsigned seqno_them, unsigned ttl,
                  const struct ProtocolParserStream *stream, unsigned secs,
                  unsigned usecs) {
  unsigned index;
...
/***************************************************************************
 ***************************************************************************/
struct TCP_Control_Block *tcpcon_lookup_tcb(struct TCP_ConnectionTable *tcpcon,
  ...
  LOG(1, "tcb_hash(0x%08x) = %s %u %s %u\n", (unsigned)index, fmt1.string,
      port_me, fmt2.string, port_them);
...
void tcp_send_RST(struct TemplatePacket *templ, struct stack_t *stack,
                  ipaddress ip_them, ipaddress ip_me, unsigned port_them,
                  unsigned port_me, unsigned seqno_them, unsigned seqno_me) {
  struct PacketBuffer *response = 0;
  ...
  response->length =
      tcp_create_packet(templ, ip_them, port_them, ip_me, port_me, seqno_me,
                        seqno_them, 0x04, /*RST*/
...
void tcpcon_send_RST(struct TCP_ConnectionTable *tcpcon, ipaddress ip_me,
                     ipaddress ip_them, unsigned port_me, unsigned port_them,
 ...
 ***************************************************************************/
static void _tcb_seg_resend(struct TCP_ConnectionTable *tcpcon,
                            struct TCP_Control_Block *tcb) {
  ...
  if (seg) {
    ...
    if (seg->is_fin && seg->length == 0) {
      ...
      tcpcon_send_packet(tcpcon, tcb, 0x18 | (seg->is_fin ? 0x01 : 0x00),
                         seg->buf, seg->length);
    }
  }
}
...
static unsigned application_notify(struct TCP_ConnectionTable *tcpcon,
                                   struct TCP_Control_Block *tcb,
                                   enum App_Event event, const void *payload,
                                   size_t payload_length, unsigned secs,
                                   unsigned usecs) {
  struct Banner1 *banner1 = tcpcon->banner1;
  const struct ProtocolParserStream *stream = tcb->stream;
  struct stack_handle_t socket = {tcpcon, tcb, secs, usecs};

  return application_event(&socket, tcb->app_state, event, stream, banner1,
                           payload, payload_length);
}

/***************************************************************************
 ***************************************************************************/
static void _tcb_seg_send(void *in_tcpcon, void *in_tcb, const void *buf,
                          size_t length, enum TCP__flags flags) {
  ...
  *next = seg;
  ...
  if (tcb->segments == seg) {
    LOGtcb(tcb, 0, "xmit = %u-bytes %s @ %u\n", length, is_fin ? "FIN" : "",
           seg->seqno - tcb->seqno_me_first);
    tcpcon_send_packet(tcpcon, tcb, 0x18 | (is_fin ? 1 : 0), seg->buf,
                       seg->length);
    if (!is_fin)
  ...
  if (length_more) {
    ...
    _tcb_seg_send(tcpcon, tcb, (unsigned char *)buf + length, length_more,
                  flags);
  }
...
size_t banner_parse(struct stack_handle_t *socket, const unsigned char *payload,
  ...
  banner1_parse(tcpcon->banner1, &tcb->banner1_state, payload, payload_length,
                &tcb->banout, socket);
...
/***************************************************************************
 ***************************************************************************/
static void _next_IP_port(struct TCP_ConnectionTable *tcpcon, ipaddress *ip_me,
                          unsigned *port_me) {
...
static void _do_reconnect(struct TCP_ConnectionTable *tcpcon,
                          ...
                          const struct ProtocolParserStream *stream,
                          unsigned secs, unsigned usecs, unsigned established) {
   ...
   */
  {
    ...
    if (ipaddress_is_equal(ip_me, prev_ip) && port_me == prev_port)
      ERRMSG(
          "There must be multiple source ports/addresses for reconnection\n");
  }
   ...
   */
  seqno =
      (unsigned)syn_cookie(ip_them, port_them, ip_me, port_me, tcpcon->entropy);

  /*
   * Now create a new TCB for this new connection
   */
  new_tcb = tcpcon_create_tcb(tcpcon, ip_me, ip_them, port_me, port_them,
                              seqno + 1, 0, 255, stream, secs, usecs);
  new_tcb->app_state = established;
}

static void _tcb_seg_close(void *in_tcpcon, void *in_tcb, unsigned secs,
                           unsigned usecs) {
  struct TCP_ConnectionTable *tcpcon = (struct TCP_ConnectionTable *)in_tcpcon;
  struct TCP_Control_Block *tcb = (struct TCP_Control_Block *)in_tcb;

  stack_incoming_tcp(tcpcon, tcb, TCP_WHAT_CLOSE, 0, 0, secs, usecs,
                     tcb->seqno_them, tcb->ackno_them);
}

/***************************************************************************
 ***************************************************************************/
int tcpapi_set_timeout(struct stack_handle_t *socket, unsigned secs,
                       unsigned usecs) {
  struct TCP_ConnectionTable *tcpcon = socket->tcpcon;
  struct TCP_Control_Block *tcb = socket->tcb;
...
int tcpapi_reconnect(struct stack_handle_t *old_socket,
                     struct ProtocolParserStream *new_stream,
                     unsigned new_app_state) {
  if (old_socket == 0 || old_socket->tcb == 0)
    return SOCKERR_EBADF;

  _do_reconnect(old_socket->tcpcon, old_socket->tcb, new_stream,
                old_socket->secs, old_socket->usecs, new_app_state);
  return 0;
}

unsigned tcpapi_change_app_state(struct stack_handle_t *socket,
                                 unsigned new_app_state) {
  struct TCP_Control_Block *tcb;
...
static void _tcb_send_ack(struct TCP_ConnectionTable *tcpcon,
                          struct TCP_Control_Block *tcb) {
  tcpcon_send_packet(tcpcon, tcb, 0x10, 0, 0);
}

static int _tcb_seg_recv(struct TCP_ConnectionTable *tcpcon,
                         struct TCP_Control_Block *tcb,
  ...
  application_notify(tcpcon, tcb, APP_RECV_PAYLOAD, payload, payload_length,
                     secs, usecs);
 ...
 *****************************************************************************/
enum TCB_result stack_incoming_tcp(
    struct TCP_ConnectionTable *tcpcon, struct TCP_Control_Block *tcb,
    enum TCP_What what, const unsigned char *payload, size_t payload_length,
    unsigned secs, unsigned usecs, unsigned seqno_them, unsigned ackno_them) {
  ...
  case STATE_SYN_SENT:
    ...
    case TCP_WHAT_SYNACK:
      ...
      break;
    default:
      ERRMSGip(tcb->ip_them, tcb->port_them,
               "%s:%s **** UNHANDLED EVENT ****\n",
  ...
  case STATE_ESTABLISHED_RECV:
    ...
    case TCP_WHAT_FIN:
      if (seqno_them == tcb->seqno_them) {
        ...
        //_tcb_send_ack(tcpcon, tcb);
        application_notify(tcpcon, tcb, APP_CLOSE, 0, payload_length, secs,
                           usecs);
    ...
    case TCP_WHAT_TIMEOUT:
      ...
      break;
    case TCP_WHAT_DATA:
      _tcb_seg_recv(tcpcon, tcb, payload, payload_length, seqno_them, secs,
                    usecs, false);
      break;
    case TCP_WHAT_SYNACK:
