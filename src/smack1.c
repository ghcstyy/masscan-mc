 ...
 ****************************************************************************/
static unsigned row_shift_from_symbol_count(unsigned symbol_count)
{
    unsigned row_shift = 1;

    symbol_count++;

    while ((unsigned) (1 << row_shift) < symbol_count) row_shift++;
...
/****************************************************************************
 ****************************************************************************/
static void create_intermediate_table(struct SMACK* smack, unsigned size)
{
    struct SmackRow* x;
...
/****************************************************************************
 ****************************************************************************/
static void destroy_intermediate_table(struct SMACK* smack)
{
...
/****************************************************************************
 ****************************************************************************/
static void create_matches_table(struct SMACK* smack, unsigned size)
{
...
/****************************************************************************
 ****************************************************************************/
static void destroy_matches_table(struct SMACK* smack)
{
    unsigned i;
 ...
 ****************************************************************************/
static void destroy_pattern_table(struct SMACK* smack)
{
    unsigned i;
 ...
 * from the new list.
 ****************************************************************************/
static unsigned id_already_exists(const size_t* ids, unsigned count, size_t new_id)
{
    unsigned i;
 ...
 ****************************************************************************/
static void smack_copy_matches(struct SmackMatches* row, const size_t* new_ids, unsigned new_count)
{
    size_t* total_ids;
    ...
    for (i = 0; i < old_count; i++) total_ids[i] = old_ids[i];
 ...
 ****************************************************************************/
static unsigned smack_add_symbol(struct SMACK* smack, unsigned c)
{
    unsigned i;
    unsigned symbol;
 ...
 ****************************************************************************/
static void smack_add_symbols(struct SMACK* smack, const unsigned char* pattern,
                              unsigned pattern_length)
{
    unsigned i;
 ...
 ****************************************************************************/
static unsigned char* make_copy_of_pattern(const unsigned char* pattern, unsigned pattern_length,
                                           unsigned is_nocase)
{
 ...
 ****************************************************************************/
void smack_add_pattern(struct SMACK* smack, const void* v_pattern, unsigned pattern_length,
                       size_t id, unsigned flags)
{
    ...
    if (smack->m_pattern_count + 1 >= smack->m_pattern_max)
    {
        ...
        if (smack->m_pattern_list)
        {
            memcpy(new_list, smack->m_pattern_list, sizeof(*new_list) * smack->m_pattern_count);
            free(smack->m_pattern_list);
        }
 ...
 ****************************************************************************/
#ifdef DEBUG
static void DEBUG_set_name(struct SMACK* smack, const void* pattern, unsigned length,
                           unsigned state)
{
...
/****************************************************************************
 ****************************************************************************/
static void smack_add_prefixes(struct SMACK* smack, struct SmackPattern* pat)
{
    unsigned i;
    unsigned pattern_length;
    unsigned char* pattern;
...
/****************************************************************************
 ****************************************************************************/
static void smack_stage0_compile_prefixes(struct SMACK* smack)
{
    unsigned s;
    unsigned a;
    ...
    {
        for (a = 0; a < ALPHABET_SIZE; a++) GOTO(s, a) = FAIL;
    }
...
/****************************************************************************
 ****************************************************************************/
static void smack_stage1_generate_fails(struct SMACK* smack)
{
    unsigned s;
    unsigned a;
    struct Queue* queue;
    ...
    while (queue_has_more_items(queue))
    {
        ...
        for (a = 0; a < ALPHABET_SIZE; a++)
        {
            ...
            while (GOTO(f, a) == FAIL) f = GOTO_FAIL(f);

            GOTO_FAIL(s) = GOTO(f, a);

            if (smack->m_match[GOTO(f, a)].m_count)
                smack_copy_matches(&smack->m_match[s], smack->m_match[GOTO(f, a)].m_ids,
...
/****************************************************************************
 ****************************************************************************/
static void smack_stage2_link_fails(struct SMACK* smack)
{
    unsigned a;
    struct Queue* queue;
...
/****************************************************************************
 ****************************************************************************/
static void smack_stage4_make_final_table(struct SMACK* smack)
{
    unsigned row;
...
/****************************************************************************
 ****************************************************************************/
static void swap_rows(struct SMACK* smack, unsigned row0, unsigned row1)
{
 ...
 ****************************************************************************/
static void smack_stage3_sort(struct SMACK* smack)
{
    unsigned start = 0;
    ...
    for (;;)
    {
        while (start < end && smack->m_match[start].m_count == 0) start++;
        while (start < end && smack->m_match[end - 1].m_count != 0) end--;
 ...
 ****************************************************************************/
static void smack_fixup_wildcards(struct SMACK* smack)
{
    size_t i;
    ...
    for (i = 0; i < smack->m_pattern_count; i++)
    {
        ...
        for (j = 0; j < pat->pattern_length; j++)
        {
            ...
            while (offset < j) smack_search_next(smack, &row, pat->pattern, &offset, (unsigned) j);
 ...
 ****************************************************************************/
static unsigned handle_match(struct SMACK* smack, unsigned index,
                             int (*callback_function)(size_t id, int index, void* callback_data),
                             void* callback_data, unsigned state)
{
    unsigned i;
...
/****************************************************************************
 ****************************************************************************/
unsigned smack_search(struct SMACK* smack, const void* v_px, unsigned length,
                      FOUND_CALLBACK cb_found, void* callback_data, unsigned* current_state)
{
...
/*****************************************************************************
 *****************************************************************************/
static size_t inner_match(const unsigned char* px, size_t length,
                          const unsigned char* char_to_symbol, const transition_t* table,
                          unsigned* state, unsigned match_limit, unsigned row_shift)
{
...
/*****************************************************************************
 *****************************************************************************/
static size_t inner_match_shift7(const unsigned char* px, size_t length,
                                 const unsigned char* char_to_symbol, const transition_t* table,
                                 unsigned* state, unsigned match_limit)
{
    const unsigned char* px_start = px;
    const unsigned char* px_end = px + length;
    unsigned row = *state;
...
/*****************************************************************************
 *****************************************************************************/
size_t smack_search_next(struct SMACK* smack, unsigned* current_state, const void* v_px,
                         unsigned* offset, unsigned length)
{
    ...
    if (current_matches == 0 /*no previous matches*/)
    {
        ...
        {
            case 7:
                i += inner_match_shift7(px + i, length - i, char_to_symbol, table, &row,
                                        match_limit);
                break;
            default:
                i += inner_match(px + i, length - i, char_to_symbol, table, &row, match_limit,
                                 row_shift);
                break;
        }
...
/****************************************************************************
 ****************************************************************************/
size_t smack_next_match(struct SMACK* smack, unsigned* current_state)
{
...
/****************************************************************************
 ****************************************************************************/
unsigned smack_search_end(struct SMACK* smack, FOUND_CALLBACK cb_found, void* callback_data,
...
size_t smack_search_next_end(struct SMACK* smack, unsigned* current_state)
{
    transition_t* table = smack->table;
    unsigned row_shift = smack->row_shift;
...
int smack_benchmark(void)
{
    ...
    for (i = 0; i < BUF_SIZE; i++) buf[i] = (char) r_rand(&seed) & 0x7F;
    ...
    for (i = 0; i < 20; i++)
    {
        ...
        for (j = 0; j < pattern_length; j++) pattern[j] = (char) (r_rand(&seed) & 0x7F) | 0x80;
    ...
    for (i = 0; i < ITERATIONS; i++)
    {
        ...
        while (offset < BUF_SIZE) result += smack_search_next(s, &state, buf, &offset, BUF_SIZE);
    }
    cycle2 = __rdtsc();
    stop = pixie_nanotime();
...
/****************************************************************************
 ****************************************************************************/
int smack_selftest(void)
{
    struct SMACK* s;
    const char* patterns[] = {
        "GET",     "PUT",        "POST",     "OPTIONS",         "HEAD",   "DELETE",
        "TRACE",   "CONNECT",    "PROPFIND", "PROPPATCH",       "MKCOL",  "MKWORKSPACE",
        "MOVE",    "LOCK",       "UNLOCK",   "VERSION-CONTROL", "REPORT", "CHECKOUT",
        "CHECKIN", "UNCHECKOUT", "COPY",     "UPDATE",          "LABEL",  "BASELINE-CONTROL",
        "MERGE",   "SEARCH",     "ACL",      "ORDERPATCH",      "PATCH",  "MKACTIVITY",
        0};
    unsigned i;
...
#define TEST(pat, offset, str)    \
    if (pat != id || offset != i) \
    return 1 + fprintf(stderr, "smack: fail %s\n", str)
