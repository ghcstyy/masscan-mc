 ...
 ****************************************************************************/
static unsigned dns_name_skip_validate(const unsigned char *px, unsigned offset,
                                       unsigned length, unsigned name_length) {
 ...
 * the names are OK.
 ****************************************************************************/
unsigned dns_name_skip(const unsigned char px[], unsigned offset,
                       unsigned max) {
...
/****************************************************************************
 ****************************************************************************/
static void dns_extract_name(const unsigned char px[], unsigned offset,
                             unsigned max, struct DomainPointer *name) {
  name->length = 0;
  ...
  for (;;) {
    ...
    if (len & 0xC0) {
      ...
      if (len == 0) {
        ...
        memcpy((unsigned char *)name->name + name->length, px + offset,
               len + 1);
...
void proto_dns_parse(struct DNS_Incoming *dns, const unsigned char px[],
  ...
  static const unsigned MAX_RRs =
      sizeof(dns->rr_offset) / sizeof(dns->rr_offset[0]);
  unsigned i;
  ...
  */
  ...
  for (i = 0; i < dns->arcount; i++) {
    ...
    /* ENDS0 OPT parsing */
    if (offset + 11 <= max && px[offset] == 0 && px[offset + 1] == 0 &&
        px[offset + 2] == 0x29) {
 ...
 ***************************************************************************/
unsigned dns_set_cookie(unsigned char *px, size_t length, uint64_t cookie) {
  if (length > 2) {
...
unsigned handle_dns(struct Output *out, time_t timestamp,
  ...
  proto_dns_parse(dns, px, parsed->app_offset,
                  parsed->app_offset + parsed->app_length);
  ...
  if (reason != 0) {
    output_report_banner(out, timestamp, ip_them, 17, port_them,
                         PROTO_DNS_VERSIONBIND, parsed->ip_ttl,
...

  {
    ...
    output_report_banner(out, timestamp, ip_them, 17, port_them,
                         PROTO_DNS_VERSIONBIND, parsed->ip_ttl, px + offset,
                         txtlen);
    return 1;
  }
}
