...
static struct Patterns html_fields[] = {{"<TiTle", 6, HTML_TITLE, 0}, {0, 0, 0, 0}};
 ...
 */
static size_t _http_append(unsigned char** inout_header, size_t length1, size_t length2,
                           const char* str)
{
...
static size_t _skip(enum What what, const unsigned char* hdr, size_t offset, size_t header_length)
{
    ...
    {
        case notspaces:
            while (offset < header_length && !isspace(hdr[offset] & 0xFF)) offset++;
            break;
        case spaces:
            ...
            {
                while (offset > 0 && hdr[offset - 1] == '\r') offset--;
            }
            break;
        case end_of_field:
            while (offset < header_length && hdr[offset] != '\n') offset++;
 ...
 */
static size_t _http_insert(unsigned char** r_hdr, size_t start, size_t end, size_t header_length,
                           size_t field_length, const void* field)
{
...
/***************************************************************************
 ***************************************************************************/
size_t http_change_requestline(unsigned char** hdr, size_t header_length, const void* field,
                               size_t field_length, int item)
{
    size_t offset;
    size_t start;
...
static size_t _field_length(const unsigned char* hdr, size_t offset, size_t hdr_length)
{
    ...
    while (offset > original_offset && isspace(hdr[offset - 1] & 0xFF)) offset--;
...
static bool _has_field_name(const char* name, size_t name_length, const unsigned char* hdr,
                            size_t offset, size_t hdr_length)
{
    size_t x;
...
/***************************************************************************
 ***************************************************************************/
size_t http_change_field(unsigned char** inout_header, size_t header_length, const char* name,
                         const unsigned char* value, size_t value_length, int what)
{
    ...
    *inout_header =
        REALLOC(*inout_header, header_length + name_length + 2 + value_length + 2 + 1 + 2);
    hdr = *inout_header;
    ...
    {
        memmove(&hdr[offset + 0], &hdr[next_offset], header_length - next_offset + 1);
    ...
    else if (what == http_field_replace)
    {
        ...
        memmove(&hdr[offset + name_length + 2 + value_length + 2], &hdr[next_offset],
...
/*****************************************************************************
 *****************************************************************************/
void field_name(struct BannerOutput* banout, size_t id, struct Patterns* xhttp_fields);
void field_name(struct BannerOutput* banout, size_t id, struct Patterns* xhttp_fields)
{
    unsigned i;
...
static void* http_init(struct Banner1* b)
{
    ...
    for (i = 0; http_fields[i].pattern; i++)
        smack_add_pattern(b->http_fields, http_fields[i].pattern, http_fields[i].pattern_length,
                          http_fields[i].id, http_fields[i].is_anchored);
    ...
    for (i = 0; html_fields[i].pattern; i++)
        smack_add_pattern(b->html_fields, html_fields[i].pattern, html_fields[i].pattern_length,
                          html_fields[i].id, html_fields[i].is_anchored);
                       ...
                       struct BannerOutput* banout, struct stack_handle_t* socket)
{
    ...
    for (i = 0; i < length; i++) switch (state)
        {
            ...
            case FIELD_NAME:
                if (px[i] == '\r')
                    break;
                id = smack_search_next(banner1->http_fields, &state2, px, &i, (unsigned) length);
                i--;
                if (id == HTTPFIELD_NEWLINE)
                {
                    state2 = 0;
            ...
            case CONTENT:
            {
                ...
                id = smack_search_next(banner1->html_fields, &state2, px, &next, (unsigned) length);
    ...
    if (state == DONE_PARSING)
        ...
        pstate->state = (state2 & 0xFFFF) << 16 | ((unsigned) id & 0xFF) << 8 | (state & 0xFF);
}
...
static int http_selftest_parser(void)
{
     ...
     * Run Test
     */
    http_parse(banner1, 0, pstate, (const unsigned char*) test_response, strlen(test_response),
               banout, 0);
...
static int http_selftest_config(void)
{
    ...
    for (i = 0; fieldsamples[i].from; i++)
    {
        ...
        len2 = http_change_field(&x, len1, "foo", (const unsigned char*) "bar", ~(size_t) 0,
                                 http_field_replace);
...
struct ProtocolParserStream banner_http = {
    "http", 80, http_hello, sizeof(http_hello) - 1, 0, http_selftest, http_init, http_parse,
};
