...
static struct Patterns html_fields[] = {{"<TiTle", 6, HTML_TITLE, 0},
                                        {0, 0, 0, 0}};
...
static size_t _skip(enum What what, const unsigned char *hdr, size_t offset,
...
static size_t _field_length(const unsigned char *hdr, size_t offset,
                            size_t hdr_length) {
  ...
  *inout_header = REALLOC(*inout_header, header_length + name_length + 2 +
  ...
  if (value == NULL || what == http_field_remove) {
    ...
    memmove(&hdr[offset + name_length + 2 + value_length + 2],
            &hdr[next_offset], header_length - offset + 1);
    header_length +=
    ...
    if (xhttp_fields[i].id == id) {
      ...
      banout_append(
          ...
          (const unsigned char *)xhttp_fields[i].pattern +
              ((xhttp_fields[i].pattern[0] == '<') ? 1
                                                   : 0), /* bah. hack. ugly. */
          xhttp_fields[i].pattern_length - ((xhttp_fields[i].pattern[0] == '<')
                                                ? 1
                                                : 0) /* bah. hack. ugly. */
      );
      return;
...
static void http_parse(const struct Banner1 *banner1, void *banner1_private,
  ...
  for (i = 0; i < length; i++)
    ...
    case FIELD_NAME:
      if (px[i] == '\r')
        break;
      id = smack_search_next(banner1->http_fields, &state2, px, &i,
                             (unsigned)length);
      i--;
      if (id == HTTPFIELD_NEWLINE) {
        state2 = 0;
    ...
    case CONTENT: {
      ...
      id = smack_search_next(banner1->html_fields, &state2, px, &next,
                             (unsigned)length);
  ...
  if (state == DONE_PARSING)
    ...
    pstate->state =
        (state2 & 0xFFFF) << 16 | ((unsigned)id & 0xFF) << 8 | (state & 0xFF);
}
...
static int http_selftest_parser(void) {
   ...
   * Run Test
   */
  http_parse(banner1, 0, pstate, (const unsigned char *)test_response,
             strlen(test_response), banout, 0);
...
struct ProtocolParserStream banner_http = {
    ...
    http_selftest, http_init, http_parse,
};
