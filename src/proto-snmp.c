 ...
 ****************************************************************************/
static uint64_t asn1_length(const unsigned char* px, uint64_t length, uint64_t* r_offset)
{
    uint64_t result;
 ...
 ****************************************************************************/
static uint64_t asn1_integer(const unsigned char* px, uint64_t length, uint64_t* r_offset)
{
    uint64_t int_length;
    uint64_t result;
    ...
    result = 0;
    while (int_length--) result = result * 256 + px[(*r_offset)++];
...
/****************************************************************************
 ****************************************************************************/
static unsigned asn1_tag(const unsigned char* px, uint64_t length, uint64_t* r_offset)
{
    if (*r_offset >= length)
        return 0;
    return px[(*r_offset)++];
...
/****************************************************************************
 ****************************************************************************/
static uint64_t next_id(const unsigned char* oid, unsigned* offset, uint64_t oid_length)
{
    uint64_t result = 0;
...
/****************************************************************************
 ****************************************************************************/
static void snmp_banner_oid(const unsigned char* oid, size_t oid_length,
                            struct BannerOutput* banout)
{
    ...
    {
        id = smack_search_next(global_mib, &state, oid, &offset, (unsigned) oid_length);
...
/****************************************************************************
 ****************************************************************************/
static void snmp_banner(const unsigned char* oid, size_t oid_length, uint64_t var_tag,
                        const unsigned char* var, size_t var_length, struct BannerOutput* banout)
{
    size_t i;
    ...
    /* print the OID */
    snmp_banner_oid(oid, oid_length, banout);
    ...
    switch (var_tag)
    {
        case 2:
        {
            char foo[32];
            uint64_t result = 0;
            for (i = 0; i < var_length; i++) result = result << 8 | var[i];
            snprintf(foo, sizeof(foo), "%" PRIu64 "", result);
            banout_append(banout, PROTO_SNMP, foo, strlen(foo));
        }
        break;
        case 6:
            snmp_banner_oid(var, var_length, banout);
            break;
        case 4:
        default:
 ...
 * newer SNMP.
 ****************************************************************************/
static void snmp_parse(const unsigned char* px, uint64_t length, struct BannerOutput* banout,
                       unsigned* request_id)
{
    uint64_t offset = 0;
 ...
 ****************************************************************************/
static unsigned convert_oid(unsigned char* dst, size_t sizeof_dst, const char* src)
{
    size_t offset = 0;
                     ...
                     struct PreprocessedInfo* parsed, uint64_t entropy)
{
    ...
    snmp_parse(px + parsed->app_offset, /* incoming SNMP response */
    ...
    output_report_banner(out, timestamp, ip_them, 17, parsed->port_src, PROTO_SNMP, parsed->ip_ttl,
...
void snmp_init(void)
{
    ...
    for (i = 0; mib[i].name; i++)
    {
        ...
        smack_add_pattern(global_mib, pattern, len, i, SMACK_ANCHOR_BEGIN | SMACK_SNMP_HACK);
    }
...
/****************************************************************************
 ****************************************************************************/
static int snmp_selftest_banner()
{
    static const unsigned char snmp_response[] = {
        0x30, 0x39, 0x02, 0x01, 0x00, 0x04, 0x06, 0x70, 0x75, 0x62, 0x6C, 0x69, 0x63, 0xA2, 0x2C,
        0x02, 0x01, 0x26, 0x02, 0x01, 0x00, 0x02, 0x01, 0x00, 0x30, 0x21, 0x30, 0x1F, 0x06, 0x09,
        0x2B, 0x06, 0x01, 0x80, 0x02, 0x01, 0x01, 0x02, 0x00, 0x06, 0x12, 0x2B, 0x06, 0x01, 0x04,
        0x01, 0x8F, 0x51, 0x01, 0x01, 0x01, 0x82, 0x29, 0x5D, 0x01, 0x1B, 0x02, 0x02, 0x01,
    };
    unsigned request_id = 0;
    struct BannerOutput banout[1];
    banout_init(banout);
    ...
    /* parse a test packet */
    snmp_parse(snmp_response, sizeof(snmp_response), banout, &request_id);
...
int snmp_selftest(void)
{
    ...
    {
        i = smack_search_next(global_mib, &state, xx, &offset, (unsigned) sizeof(xx));
