...
/****************************************************************************
 ****************************************************************************/
static void snmp_banner(const unsigned char *oid, size_t oid_length,
                        uint64_t var_tag, const unsigned char *var,
                        size_t var_length, struct BannerOutput *banout) {
  size_t i;
...
unsigned handle_snmp(struct Output *out, time_t timestamp,
  ...
  output_report_banner(out, timestamp, ip_them, 17, parsed->port_src,
                       PROTO_SNMP, parsed->ip_ttl,
...
static int snmp_selftest_banner() {
  static const unsigned char snmp_response[] = {
      0x30, 0x39, 0x02, 0x01, 0x00, 0x04, 0x06, 0x70, 0x75, 0x62, 0x6C, 0x69,
      0x63, 0xA2, 0x2C, 0x02, 0x01, 0x26, 0x02, 0x01, 0x00, 0x02, 0x01, 0x00,
      0x30, 0x21, 0x30, 0x1F, 0x06, 0x09, 0x2B, 0x06, 0x01, 0x80, 0x02, 0x01,
      0x01, 0x02, 0x00, 0x06, 0x12, 0x2B, 0x06, 0x01, 0x04, 0x01, 0x8F, 0x51,
...

  {
    ...
    if (memcmp(str, "sysObjectID:okidata.1.1.1.297.93.1.27.2.2.1",
               str_length) != 0)
      return 1;
  }
...
int snmp_selftest(void) {
  ...
  while (offset < sizeof(xx)) {
    i = smack_search_next(global_mib, &state, xx, &offset,
                          (unsigned)sizeof(xx));
