...
const char *reason_string(int x, char *buffer, size_t sizeof_buffer) {
  snprintf(buffer, sizeof_buffer, "%s%s%s%s%s%s%s%s", (x & 0x01) ? "fin-" : "",
           (x & 0x02) ? "syn-" : "", (x & 0x04) ? "rst-" : "",
           (x & 0x08) ? "psh-" : "", (x & 0x10) ? "ack-" : "",
           (x & 0x20) ? "urg-" : "", (x & 0x40) ? "ece-" : "",
           (x & 0x80) ? "cwr-" : "");
  if (buffer[0] == '\0')
    return "none";
  else
 ...
 *****************************************************************************/
static time_t next_rotate_time(time_t last_rotate, unsigned period,
                               unsigned offset) {
  time_t next;
...
static char *indexed_filename(const char *filename, unsigned index) {
  ...
  /* format the new name */
  snprintf(new_filename, new_length, "%.*s.%02u%s", (unsigned)ext, filename,
           index, filename + ext);
...
struct Output *output_create(const struct Masscan *masscan,
  ...
  if (masscan->output.rotate.timeout == 0) {
    ...
    if (out->rotate.offset > 1) {
      out->rotate.next =
          next_rotate_time(out->rotate.last - out->rotate.period,
...
static FILE *output_do_rotate(struct Output *out, int is_closing) {
  ...
  new_filename_size = strlen(dir) + strlen("/") + strlen(filename) +
                      strlen("1308201101-") + strlen(filename) + 1 /* - */
                      + 1;                                         /* null */
...
again:
  if (out->rotate.filesize) {
    ...
    snprintf(new_filename, new_filename_size, "%s/%.*s-%05u%.*s", dir,
             (unsigned)x_off, filename, out->rotate.filecount++,
             (unsigned)x_len, filename + x_off);
  } else {
    snprintf(new_filename, new_filename_size,
             "%s/%02u%02u%02u-%02u%02u%02u"
             "-%s",
             dir, tm.tm_year % 100, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
             tm.tm_min, tm.tm_sec, filename);
  ...
  if (out->rotate.period) {
    out->rotate.next =
        next_rotate_time(time(0), out->rotate.period, out->rotate.offset);
  }
 ...
 * back.
 ***************************************************************************/
void output_report_status(struct Output *out, time_t timestamp, int status,
  ...
  if (out->is_interactive || out->format == 0 ||
    ...
    case 0: /* ARP */
      count = fprintf(
          stdout,
          "Discovered %s port %u/%s on %s (%02x:%02x:%02x:%02x:%02x:%02x) %s",
          status_string(status), port, name_from_ip_proto(ip_proto), fmt.string,
          mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], oui_from_mac(mac));
      break;
    default:
      count = fprintf(stdout, "Discovered %s port %u/%s on %s",
                      status_string(status), port, name_from_ip_proto(ip_proto),
                      fmt.string);
    }
   ...
   * and so on.
   */
  out->funcs->status(out, fp, timestamp, status, ip, ip_proto, port, reason,
                     ttl);
}

/***************************************************************************
 ***************************************************************************/
void output_report_banner(struct Output *out, time_t now, ipaddress ip,
                          unsigned ip_proto, unsigned port, unsigned proto,
  ...
  if (out->is_interactive || out->format == 0 ||
    ...
    count = fprintf(
        stdout, "Banner on port %u/%s on %s: [%s] %s", port,
