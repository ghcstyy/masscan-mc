 ...
 *****************************************************************************/
const char* status_string(enum PortStatus status)
{
    switch (status)
    {
...
{
    snprintf(buffer, sizeof_buffer, "%s%s%s%s%s%s%s%s", (x & 0x01) ? "fin-" : "",
             (x & 0x02) ? "syn-" : "", (x & 0x04) ? "rst-" : "", (x & 0x08) ? "psh-" : "",
             (x & 0x10) ? "ack-" : "", (x & 0x20) ? "urg-" : "", (x & 0x40) ? "ece-" : "",
             (x & 0x80) ? "cwr-" : "");
    if (buffer[0] == '\0')
        return "none";
    else
 ...
 *****************************************************************************/
static FILE* open_rotate(struct Output* out, const char* filename)
{
    FILE* fp = 0;
 ...
 *****************************************************************************/
static void close_rotate(struct Output* out, FILE* fp)
{
    if (out == NULL)
        return;
    if (fp == NULL)
 ...
 *****************************************************************************/
static time_t next_rotate_time(time_t last_rotate, unsigned period, unsigned offset)
{
    time_t next;
 ...
 *****************************************************************************/
static char* indexed_filename(const char* filename, unsigned index)
{
    ...
    /* format the new name */
    snprintf(new_filename, new_length, "%.*s.%02u%s", (unsigned) ext, filename, index,
             filename + ext);
 ...
 *****************************************************************************/
struct Output* output_create(const struct Masscan* masscan, unsigned thread_index)
{
    struct Output* out;
    unsigned i;
    ...
    else
    {
        ...
        {
            out->rotate.next = next_rotate_time(out->rotate.last - out->rotate.period,
                                                out->rotate.period, out->rotate.offset);
        }
        else
        {
            out->rotate.next =
                next_rotate_time(out->rotate.last, out->rotate.period, out->rotate.offset);
        }
    }
 ...
 *****************************************************************************/
static FILE* output_do_rotate(struct Output* out, int is_closing)
{
    const char* dir;
    const char* filename;
    char* new_filename;
    ...
    new_filename_size = strlen(dir) + strlen("/") + strlen(filename) + strlen("1308201101-") +
                        strlen(filename) + 1 /* - */
                        + 1;                 /* null */
...
again:
    if (out->rotate.filesize)
    {
        ...
        snprintf(new_filename, new_filename_size, "%s/%.*s-%05u%.*s", dir, (unsigned) x_off,
                 filename, out->rotate.filecount++, (unsigned) x_len, filename + x_off);
    }
    else
    {
        snprintf(new_filename, new_filename_size,
                 "%s/%02u%02u%02u-%02u%02u%02u"
                 "-%s",
                 dir, tm.tm_year % 100, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec,
                 filename);
    ...
    {
        out->rotate.next = next_rotate_time(time(0), out->rotate.period, out->rotate.offset);
    }
...
/***************************************************************************
 ***************************************************************************/
static int is_rotate_time(const struct Output* out, time_t now, FILE* fp)
{
    if (out->is_virgin_file)
        return 0;
    if (now >= out->rotate.next)
        return 1;
    if (out->rotate.filesize != 0 && ftell_x(fp) >= (int64_t) out->rotate.filesize)
        return 1;
    return 0;
}
 ...
 ***************************************************************************/
static const char* oui_from_mac(const unsigned char mac[6])
{
 ...
 * back.
 ***************************************************************************/
void output_report_status(struct Output* out, time_t timestamp, int status, ipaddress ip,
                          unsigned ip_proto, unsigned port, unsigned reason, unsigned ttl,
                          const unsigned char mac[6])
{
    ...
    if (out->is_interactive || out->format == 0 || out->format == Output_Interactive)
    {
        ...
        switch (ip_proto)
        {
            case 0: /* ARP */
                count = fprintf(
                    stdout, "Discovered %s port %u/%s on %s (%02x:%02x:%02x:%02x:%02x:%02x) %s",
                    status_string(status), port, name_from_ip_proto(ip_proto), fmt.string, mac[0],
                    mac[1], mac[2], mac[3], mac[4], mac[5], oui_from_mac(mac));
                break;
            default:
                count = fprintf(stdout, "Discovered %s port %u/%s on %s", status_string(status),
                                port, name_from_ip_proto(ip_proto), fmt.string);
        }
...
/***************************************************************************
 ***************************************************************************/
void output_report_banner(struct Output* out, time_t now, ipaddress ip, unsigned ip_proto,
                          unsigned port, unsigned proto, unsigned ttl, const unsigned char* px,
                          unsigned length)
{
    FILE* fp = out->fp;
    ipaddress_formatted_t fmt = ipaddress_fmt(ip);
    ...
    if (out->is_interactive || out->format == 0 || out->format == Output_Interactive)
    {
        ...
        count = fprintf(stdout, "Banner on port %u/%s on %s: [%s] %s", port,
                        name_from_ip_proto(ip_proto), fmt.string, masscan_app_to_string(proto),
