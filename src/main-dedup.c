 ...
 */
void dedup_destroy(struct DedupTable *dedup) { free(dedup); }
 ...
 */
static inline unsigned dedup_hash_ipv6(ipaddress ip_them, unsigned port_them,
 ...
 */
static inline void swap6(struct DedupEntry_IPv6 *lhs,
 ...
 */
static unsigned dedup_is_duplicate_ipv6(struct DedupTable *dedup,
   ...
   * seen in a while. */
  for (i = 0; i < 4; i++) {
    if (is_equal6(bucket[i].ip_them, ip_them.ipv6) &&
        bucket[i].port_them == port_them &&
        is_equal6(bucket[i].ip_me, ip_me.ipv6) &&
        bucket[i].port_me == port_me) {
...
/***************************************************************************
 ***************************************************************************/
static unsigned dedup_is_duplicate_ipv4(struct DedupTable *dedup,
                                        ipaddress ip_them, unsigned port_them,
                                        ipaddress ip_me, unsigned port_me) {
  unsigned hash;
