 ...
 ***************************************************************************/
int masscan_initialize_adapter(struct Masscan* masscan, unsigned index, macaddress_t* source_mac,
                               macaddress_t* router_mac_ipv4, macaddress_t* router_mac_ipv6)
{
    char* ifname;
    char ifname2[256];
    unsigned adapter_ip = 0;
    unsigned is_usable_ipv4 = !massip_has_ipv4_targets(
        &masscan->targets); /* I don't understand this line, seems opposite */
    unsigned is_usable_ipv6 = !massip_has_ipv6_targets(
        &masscan->targets); /* I don't understand this line, seems opposite */
    ...
    masscan->nic[index].adapter = rawsock_init_adapter(
        ...
        masscan->is_offline, (void*) masscan->bpf_filter, masscan->nic[index].is_vlan,
    ...
    if (massip_has_ipv4_targets(&masscan->targets))
    {
        ...
        if (adapter_ip == 0)
        {
            ...
            LOG(0,
                "    [hint] if it has no IP address, manually set with something "
                "like "
        ...
        *router_mac_ipv4 = masscan->nic[index].router_mac_ipv4;
        ...
        else if (macaddress_is_zero(*router_mac_ipv4))
        {
            ...
            if (err == 0)
            {
                ...
                stack_arp_resolve(masscan->nic[index].adapter, adapter_ip, *source_mac, router_ipv4,
                                  router_mac_ipv4);
            }
            ...
            if (macaddress_is_zero(*router_mac_ipv4))
            {
                ...
                LOG(0,
                    ...
                    ifname, fmt.string);
                LOG(0,
                    "    [hint] try \"--router ip 192.0.2.1\" to specify different "
                    "router\n");
                LOG(0,
                    "    [hint] try \"--router-mac 66-55-44-33-22-11\" instead to "
                    "bypass ARP\n");
    ...
    if (massip_has_ipv6_targets(&masscan->targets))
    {
        ...
        *router_mac_ipv6 = masscan->nic[index].router_mac_ipv6;
        ...
        if (macaddress_is_zero(*router_mac_ipv6))
        {
             ...
             * some time */
            stack_ndpv6_resolve(masscan->nic[index].adapter, adapter_ipv6, *source_mac,
                                router_mac_ipv6);
        }
        ...
        if (macaddress_is_zero(*router_mac_ipv6))
        {
            ...
            LOG(0,
                ...
                ifname, fmt.string);
            LOG(0,
                "    [hint] try \"--router-mac-ipv6 66-55-44-33-22-11\" instead "
                "to bypass ARP\n");
            LOG(0, "    [hint] try \"--interface eth0\" to change interface\n");
            return -1;
        }
