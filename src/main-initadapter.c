 ...
 ***************************************************************************/
int masscan_initialize_adapter(struct Masscan *masscan, unsigned index,
  ...
  if (massip_has_ipv4_targets(&masscan->targets)) {
    ...
    if (adapter_ip == 0) {
      ...
      LOG(0, "    [hint] if it has no IP address, manually set with something "
             "like "
    ...
    if (masscan->is_offline) {
      ...
      if (macaddress_is_zero(*router_mac_ipv4)) {
        ...
        LOG(0,
            ...
            "\"%s\"\n",
            ifname, fmt.string);
        LOG(0, "    [hint] try \"--router ip 192.0.2.1\" to specify different "
               "router\n");
        LOG(0, "    [hint] try \"--router-mac 66-55-44-33-22-11\" instead to "
               "bypass ARP\n");
    ...
    *router_mac_ipv6 = masscan->nic[index].router_mac_ipv6;
    ...
    if (macaddress_is_zero(*router_mac_ipv6)) {
      ...
      LOG(0,
          ...
          "\"%s\"\n",
          ifname, fmt.string);
      LOG(0, "    [hint] try \"--router-mac-ipv6 66-55-44-33-22-11\" instead "
             "to bypass ARP\n");
      LOG(0, "    [hint] try \"--interface eth0\" to change interface\n");
      return -1;
    }
