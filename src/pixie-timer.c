...
int win_clock_gettime(int X, struct timeval *tv) {
  LARGE_INTEGER t;
  FILETIME f;
  double microseconds;
...
void pixie_mssleep(unsigned waitTime) { Sleep(waitTime); }

void pixie_usleep(uint64_t waitTime) {
  /*
...
uint64_t pixie_gettime(void) { return mach_absolute_time() / 1000; }
uint64_t pixie_nanotime(void) { return mach_absolute_time(); }
#endif
