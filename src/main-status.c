...
void status_print(struct Status *status, uint64_t count, uint64_t max_count,
                  ...
                  uint64_t total_syns, uint64_t exiting, bool json_status) {
  double elapsed_time;
  double rate;
  double now;
  double percent_done;
  ...
  if (status->is_infinite) {
    ...
    fprintf(stderr, fmt, kpps, syn_rate, synack_rate, tcb_rate, total_tcbs, pps,
            count);
  } else {
    if (is_tx_done) {
      if (json_status == 1)
        ...
        fmt = "rate:%6.2f-kpps, %5.2f%% done, waiting %d-secs, found=%" PRIu64
              "       \r";

      fprintf(stderr, fmt, pps / 1000.0, percent_done, (int)exiting,
      ...
      fprintf(stderr, fmt, pps / 1000.0, percent_done,
              (unsigned)(time_remaining / 60 / 60),
              (unsigned)(time_remaining / 60) % 60,
              (unsigned)(time_remaining) % 60, total_synacks, pps, count,
