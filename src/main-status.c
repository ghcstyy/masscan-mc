 ...
 * packet is slow.
 ***************************************************************************/
void status_print(struct Status* status, uint64_t count, uint64_t max_count, double pps,
                  uint64_t total_tcbs, uint64_t total_synacks, uint64_t total_syns,
                  uint64_t exiting, bool json_status)
{
    ...
    /**
     * {"state":"*","rate":{"kpps":24.99,"pps":24985.49,"synps":
     * 27763,"ackps":4,"tcbps":4},"tcb": 33,"syn":246648}
     */
    ...
    /**
     * {"state":"waiting","rate":{"kpps":0.00,"pps":0.00},"progress":{"percent":21.87,"seconds":4,"found":56,"syn":{"sent":
     * 341436,"total":1561528,"remaining":1220092}}}
     */
     ...
     *      "hours":0,"mins":0,"seconds":55},"syn":{"sent": 152510,"total":
     * 1561528,"remaining": 1409018},"found": 27}}
     */
    ...
    if (status->is_infinite)
    {
        if (json_status == 1)
            ...
            fmt = "rate:%6.2f-kpps, syn/s=%.0f ack/s=%.0f tcb-rate=%.0f, %" PRIu64
                  "-tcbs,         \r";

        fprintf(stderr, fmt, kpps, syn_rate, synack_rate, tcb_rate, total_tcbs, pps, count);
    }
    else
    {
        if (is_tx_done)
        {
            ...
            fprintf(stderr, fmt, pps / 1000.0, percent_done, (int) exiting, total_synacks, pps,
        ...
        else
        {
            if (json_status == 1)
                ...
                fmt =
                    "rate:%6.2f-kpps, %5.2f%% done,%4u:%02u:%02u remaining, "
                    "found=%" PRIu64 "       \r";

            fprintf(stderr, fmt, pps / 1000.0, percent_done, (unsigned) (time_remaining / 60 / 60),
                    (unsigned) (time_remaining / 60) % 60, (unsigned) (time_remaining) % 60,
                    total_synacks, pps, count, max_count, max_count - count);
        }
    }
    fflush(stderr);
