...
static unsigned null_PCAP_DISPATCH(void* hPcap, unsigned how_many_packets,
                                   PCAP_HANDLE_PACKET handler, void* handle_data)
{
#ifdef STATICPCAP
    return pcap_dispatch(hPcap, how_many_packets, handler, handle_data);
#endif
    UNUSEDPARM(hPcap);
    UNUSEDPARM(how_many_packets);
    UNUSEDPARM(handler);
    UNUSEDPARM(handle_data);
    return 0;
}
...
#endif
    seterr(errbuf, "libpcap not loaded");
    UNUSEDPARM(devicename);
    UNUSEDPARM(snap_length);
    UNUSEDPARM(is_promiscuous);
    UNUSEDPARM(read_timeout);
    return NULL;
}
...
static int null_PCAP_SENDQUEUE_QUEUE(pcap_send_queue* queue, const struct pcap_pkthdr* pkt_header,
...
#define DOLINK(PCAP_DATALINK, datalink)                                         \
...

    {
        static const char* possible_names[] = {
            "libpcap.so",       "libpcap.A.dylib",  "libpcap.dylib",  "libpcap.so.1",
            "libpcap.so.0.9.5", "libpcap.so.0.9.4", "libpcap.so.0.8", 0};
        unsigned i;
...
#define DOLINK(PCAP_DATALINK, datalink)                                \
    ...
    if (pl->datalink == NULL)                                          \
        LOG(1, "pcap: pcap_%s: failed\n", #datalink);                  \
    if (pl->datalink == NULL)                                          \
        pl->func_err = 1, pl->datalink = null_##PCAP_DATALINK;
#else
#define DOLINK(PCAP_DATALINK, datalink) pl->func_err = 0, pl->datalink = null_##PCAP_DATALINK;
#endif
#endif
