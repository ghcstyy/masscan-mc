 ...
 */
typedef void (*PCAP_HANDLE_PACKET)(unsigned char* v_seap, const struct pcap_pkthdr* framehdr,
                                   const unsigned char* buf);
typedef void (*PCAP_CLOSE)(void* hPcap);
typedef unsigned (*PCAP_DATALINK)(void* hPcap);
typedef unsigned (*PCAP_DISPATCH)(void* hPcap, unsigned how_many_packets,
...
typedef void* (*PCAP_OPEN_LIVE)(const char* devicename, unsigned snap_length,
...
typedef int (*PCAP_SENDQUEUE_QUEUE)(pcap_send_queue* queue, const struct pcap_pkthdr* pkt_header,
