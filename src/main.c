 ...
 ***************************************************************************/
static void adapter_get_source_addresses(const struct Masscan *masscan,
                                         unsigned nic_index,
                                         struct source_t *src) {
...
infinite:
  ...
  start = masscan->resume.index +
          (masscan->shard.one - 1) * masscan->nic_count + parms->nic_index;
  end = range;
  ...
  *status_tcb_count = 0;
  ...
  if (masscan->is_banners) {
    ...
    tcpcon = tcpcon_create_table(
        (size_t)((masscan->max_rate / 5) / masscan->nic_count), parms->stack,
        &parms->tmplset->pkts[Proto_TCP], output_report_banner, out,
    ...
    tcpcon_set_banner_flags(
        tcpcon, masscan->is_capture_cert, masscan->is_capture_servername,
    ...
    if (masscan->http.user_agent)
      tcpcon_set_parameter(tcpcon, "http-user-agent",
    ...
    if (masscan->http.method)
      tcpcon_set_parameter(tcpcon, "http-method", masscan->http.method_length,
                           masscan->http.method);
    if (masscan->http.url)
      tcpcon_set_parameter(tcpcon, "http-url", masscan->http.url_length,
                           masscan->http.url);
    ...
    for (pay = masscan->payloads.tcp; pay; pay = pay->next) {
      ...
      tcpcon_set_parameter(tcpcon, name, strlen(pay->payload_base64),
                           pay->payload_base64);
    }
  }
  ...
  while (!is_rx_done) {
...

    {
      ...
      LOGip(5, ip_them, port_them, "-> TCP ackno=0x%08x flags=0x%02x(%s)\n",
            ...
            reason_string(TCP_FLAGS(px, parsed.transport_offset), buf,
                          sizeof(buf)));
    }
      ...
      if (TCP_IS_SYNACK(px, parsed.transport_offset)) {
        ...
        if (tcb == NULL) {
          tcb = tcpcon_create_tcb(tcpcon, ip_me, ip_them, port_me, port_them,
                                  seqno_me, seqno_them + 1, parsed.ip_ttl, NULL,
                                  secs, usecs);
          (*status_tcb_count)++;
        }
...
static void control_c_handler(int x) {
  ...
  if (control_c_pressed == 0) {
    fprintf(stderr, "waiting several seconds to exit..."
                    "                                            \n");
    fflush(stderr);
...
static int main_scan(struct Masscan *masscan) {
  ...
  count_ips = rangelist_count(&masscan->targets.ipv4) +
              range6list_count(&masscan->targets.ipv6).lo;
  if (count_ips == 0) {
  ...
  for (index = 0; index < masscan->nic_count; index++) {
    ...
    template_packet_init(parms->tmplset, parms->source_mac,
                         ...
                         masscan->seed, masscan->templ_opts);
  ...
  while (!is_tx_done && masscan->output.is_status_updates) {
    ...
    if (masscan->output.is_status_updates)
      status_print(&status, min_index, range, rate, total_tcbs, total_synacks,
                   total_syns, 0, masscan->output.is_status_ndjson);
  ...
  for (;;) {
    ...
    if (masscan->output.is_status_updates) {
      status_print(&status, min_index, range, rate, total_tcbs, total_synacks,
                   total_syns, masscan->wait - (time(0) - now),
...
int main(int argc, char *argv[]) {
  ...
  case Operation_ReadScan: {
    ...
    /* find last file */
    for (stop = start + 1; stop < (unsigned)argc && argv[stop][0] != '-';
         stop++)
      ;
