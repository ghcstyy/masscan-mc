 ...
 ***************************************************************************/
static void adapter_get_source_addresses(const struct Masscan* masscan, unsigned nic_index,
                                         struct source_t* src)
{
...
infinite:
    ...
    start =
        masscan->resume.index + (masscan->shard.one - 1) * masscan->nic_count + parms->nic_index;
    end = range;
    ...
    for (i = start; i < end;)
    {
        ...
        while (batch_size && i < end)
        {
            ...
            if (xXx < range_ipv6)
            {
                ...
                rawsock_send_probe_ipv6(adapter, ip_them, port_them, ip_me, port_me,
                                        (unsigned) cookie,
            ...
            else
            {
                ...
                {
                    uint64_t ck =
                        syn_cookie_ipv4((unsigned) (i + repeats), (unsigned) ((i + repeats) >> 32),
                 ...
                 */
                rawsock_send_probe_ipv4(adapter, ip_them, port_them, ip_me, port_me,
                                        (unsigned) cookie,
                                        !batch_size, /* flush queue on last packet in batch */
                                        &pkt_template);
            }
...
static void receive_thread(void* v)
{
    ...
    *status_tcb_count = 0;
    ...
    if (masscan->is_banners)
    {
         ...
         */
        tcpcon =
            tcpcon_create_table((size_t) ((masscan->max_rate / 5) / masscan->nic_count),
                                parms->stack, &parms->tmplset->pkts[Proto_TCP],
                                output_report_banner, out, masscan->tcb.timeout, masscan->seed);
         ...
         */
        tcpcon_set_banner_flags(tcpcon, masscan->is_capture_cert, masscan->is_capture_servername,
                                masscan->is_capture_html, masscan->is_capture_heartbleed,
        ...
        if (masscan->http.payload)
            tcpcon_set_parameter(tcpcon, "http-payload", masscan->http.payload_length,
                                 masscan->http.payload);
        if (masscan->http.user_agent)
            tcpcon_set_parameter(tcpcon, "http-user-agent", masscan->http.user_agent_length,
                                 masscan->http.user_agent);
        if (masscan->http.host)
            tcpcon_set_parameter(tcpcon, "http-host", masscan->http.host_length,
                                 masscan->http.host);
        if (masscan->http.method)
            tcpcon_set_parameter(tcpcon, "http-method", masscan->http.method_length,
                                 masscan->http.method);
        if (masscan->http.url)
            tcpcon_set_parameter(tcpcon, "http-url", masscan->http.url_length, masscan->http.url);
        if (masscan->http.version)
            tcpcon_set_parameter(tcpcon, "http-version", masscan->http.version_length,
        ...
        if (masscan->tcp_connection_timeout)
        {
            ...
            tcpcon_set_parameter(tcpcon, "timeout", strlen(foo), foo);
        }
        if (masscan->tcp_hello_timeout)
        {
            char foo[64];
            snprintf(foo, sizeof(foo), "%u", masscan->tcp_hello_timeout);
            tcpcon_set_parameter(tcpcon, "hello-timeout", strlen(foo), foo);
        }

        for (i = 0; i < masscan->http.headers_count; i++)
        {
            tcpcon_set_http_header(tcpcon, masscan->http.headers[i].name,
                                   masscan->http.headers[i].value_length,
                                   masscan->http.headers[i].value, http_field_replace);
        }
        for (i = 0; i < masscan->http.cookies_count; i++)
        {
            tcpcon_set_http_header(tcpcon, "Cookie", masscan->http.cookies[i].value_length,
                                   masscan->http.cookies[i].value, http_field_add);
        }
        for (i = 0; i < masscan->http.remove_count; i++)
        {
            tcpcon_set_http_header(tcpcon, masscan->http.headers[i].name, 0, 0, http_field_remove);
        }
        ...
        for (pay = masscan->payloads.tcp; pay; pay = pay->next)
        {
            ...
            tcpcon_set_parameter(tcpcon, name, strlen(pay->payload_base64), pay->payload_base64);
        }
    }
    ...
    {
        while (!is_rx_done) pixie_usleep(10000);
    ...
    while (!is_rx_done)
    {
        ...
        switch (parsed.ip_protocol)
        {
            case 132: /* SCTP */
                cookie =
                    syn_cookie(ip_them, port_them | (Proto_SCTP << 16), ip_me, port_me, entropy) &
                    0xFFFFFFFF;
                break;
            default:
            ...
            case FOUND_ARP:
                ...
                switch (parsed.opcode)
                {
                    case 1: /* request */
                        ...
                        stack_arp_incoming_request(stack, ip_me.ipv4, parms->source_mac, px,
                                                   length);
                        break;
                    case 2: /* response */
        ...
        if (tcpcon)
        {
            ...
            if (TCP_IS_SYNACK(px, parsed.transport_offset))
            {
                ...
                if (tcb == NULL)
                {
                    tcb = tcpcon_create_tcb(tcpcon, ip_me, ip_them, port_me, port_them, seqno_me,
                                            seqno_them + 1, parsed.ip_ttl, NULL, secs, usecs);
                    (*status_tcb_count)++;
                }
                Q += stack_incoming_tcp(tcpcon, tcb, TCP_WHAT_SYNACK, 0, 0, secs, usecs,
            ...
            else if (tcb)
            {
                ...
                {
                    Q += stack_incoming_tcp(tcpcon, tcb, TCP_WHAT_ACK, 0, 0, secs, usecs,
                                            seqno_them, seqno_me);
                }
                ...
                {
                    Q += stack_incoming_tcp(tcpcon, tcb, TCP_WHAT_DATA, px + parsed.app_offset,
                                            parsed.app_length, secs, usecs, seqno_them, seqno_me);
                }
                ...
                {
                    Q += stack_incoming_tcp(
                        tcpcon, tcb, TCP_WHAT_FIN, 0, 0, secs, usecs,
                        seqno_them + parsed.app_length, /* the FIN comes after any data in
                                                           the packet */
                        seqno_me);
                }
                ...
                {
                    Q += stack_incoming_tcp(tcpcon, tcb, TCP_WHAT_RST, 0, 0, secs, usecs,
                                            seqno_them, seqno_me);
                }
            }
            else if (TCP_IS_FIN(px, parsed.transport_offset))
            {
                ...
                else
                {
                    ...
                    if (!is_suppress)
                        tcpcon_send_RST(tcpcon, ip_me, ip_them, port_me, port_them, seqno_them,
                                        seqno_me);
                }
            }
        }
        ...
        if (TCP_IS_SYNACK(px, parsed.transport_offset) || TCP_IS_RST(px, parsed.transport_offset))
        {
             ...
             */
            output_report_status(out, global_now, status, ip_them, 6,         /* ip proto = tcp */
                                 port_them, px[parsed.transport_offset + 13], /* tcp flags */
            ...
            if (tcpcon == NULL && !masscan->is_noreset)
                tcp_send_RST(&parms->tmplset->pkts[Proto_TCP], parms->stack, ip_them, ip_me,
                             port_them, port_me, 0, seqno_me);
        }
    }
...
static int main_scan(struct Masscan* masscan)
{
     ...
     */
    count_ips =
        rangelist_count(&masscan->targets.ipv4) + range6list_count(&masscan->targets.ipv6).lo;
    if (count_ips == 0)
    {
    ...
    if (count_ips > 1000000000ULL && rangelist_count(&masscan->exclude.ipv4) == 0)
    {
        ...
        LOG(0,
            " [hint] to prevent accidents, at least one --exclude must be "
            "specified\n");
    ...
    for (index = 0; index < masscan->nic_count; index++)
    {
        ...
        template_packet_init(
            parms->tmplset, parms->source_mac, parms->router_mac_ipv4, parms->router_mac_ipv6,
            masscan->payloads.udp, masscan->payloads.oproto,
            stack_if_datalink(masscan->nic[index].adapter), masscan->seed, masscan->templ_opts);
     ...
     */
    {
        ...
        if (count_ports == 1 && masscan->targets.ports.list->begin == Templ_ICMP_echo &&
        ...
        { /* ICMP only */
            ...
            LOG(0, "Scanning %u hosts\n", (unsigned) count_ips);
              ...
              */
        {
            // LOG(0, " -- forced options: -sS -Pn -n --randomize-hosts -v
            // --send-eth\n");
    ...
    for (;;)
    {
        ...
        if (masscan->output.is_status_updates)
        {
            status_print(&status, min_index, range, rate, total_tcbs, total_synacks, total_syns,
                         masscan->wait - (time(0) - now), masscan->output.is_status_ndjson);
...
int main(int argc, char* argv[])
{
    ...
    safe_strcpy(masscan->output.rotate.directory, sizeof(masscan->output.rotate.directory), ".");
    ...
    has_target_addresses =
        massip_has_ipv4_targets(&masscan->targets) || massip_has_ipv6_targets(&masscan->targets);
    ...
    switch (masscan->op)
    {
        ...
        case Operation_Scan:
            ...
            if (rangelist_count(&masscan->targets.ipv4) == 0 &&
                massint128_is_zero(range6list_count(&masscan->targets.ipv6)))
            {
        ...
        case Operation_DebugIF:
            for (i = 0; i < masscan->nic_count; i++) rawsock_selftest_if(masscan->nic[i].ifname);
            return 0;
