 ...
 * function prototypes
 */
typedef pfring *(*PFRING_OPEN)(const char *device_name, unsigned caplen,
                               unsigned flags);
typedef void (*PFRING_CLOSE)(pfring *ring);
typedef int (*PFRING_ENABLE_RING)(pfring *ring);
typedef int (*PFRING_SEND)(pfring *ring, const unsigned char *buffer,
                           unsigned buffer_length, unsigned char flush_packet);
typedef int (*PFRING_RECV)(pfring *ring, unsigned char **buffer,
                           unsigned buffer_length, struct pfring_pkthdr *hdr,
...
typedef int (*PFRING_GET_BOUND_DEVICE)(pfring *ring,
