 ...
 ***************************************************************************/
static unsigned ip_header_checksum(const unsigned char* px, unsigned offset, unsigned max_offset)
{
...
/***************************************************************************
 ***************************************************************************/
static unsigned tcp_checksum2(const unsigned char* px, unsigned offset_ip, unsigned offset_tcp,
                              size_t tcp_length)
{
    uint64_t xsum = 0;
    unsigned i;
    ...
    xsum -= (tcp_length & 1) *
            px[offset_tcp + i - 1]; /* yea I know going off end of packet is bad so sue me */
    xsum = (xsum & 0xFFFF) + (xsum >> 16);
    xsum = (xsum & 0xFFFF) + (xsum >> 16);
    xsum = (xsum & 0xFFFF) + (xsum >> 16);
...
/***************************************************************************
 ***************************************************************************/
unsigned udp_checksum2(const unsigned char* px, unsigned offset_ip, unsigned offset_tcp,
                       size_t tcp_length)
{
    uint64_t xsum = 0;
    unsigned i;
...
{
    return udp_checksum2(tmpl->ipv4.packet, tmpl->ipv4.offset_ip, tmpl->ipv4.offset_tcp,
                         tmpl->ipv4.length - tmpl->ipv4.offset_tcp);
}

/***************************************************************************
 ***************************************************************************/
static unsigned icmp_checksum2(const unsigned char* px, unsigned offset_icmp, size_t icmp_length)
{
    uint64_t xsum = 0;
    unsigned i;
    ...
    xsum -= (icmp_length & 1) *
            px[offset_icmp + i - 1]; /* yea I know going off end of packet is bad so sue me */
    xsum = (xsum & 0xFFFF) + (xsum >> 16);
    xsum = (xsum & 0xFFFF) + (xsum >> 16);
    xsum = (xsum & 0xFFFF) + (xsum >> 16);
...
void tcp_set_window(unsigned char* px, size_t px_length, unsigned window)
{
    ...
    xsum = ~tcp_checksum2(px, parsed.ip_offset, parsed.transport_offset, parsed.transport_length);
...
/***************************************************************************
 ***************************************************************************/
size_t tcp_create_packet(struct TemplatePacket* tmpl, ipaddress ip_them, unsigned port_them,
                         ipaddress ip_me, unsigned port_me, unsigned seqno, unsigned ackno,
                         unsigned flags, const unsigned char* payload, size_t payload_length,
...
/***************************************************************************
 ***************************************************************************/
static void udp_payload_fixup(struct TemplatePacket* tmpl, unsigned port, unsigned seqno)
{
    ...
    payloads_udp_lookup(tmpl->payloads, port, &px2, &length2, &source_port2, &xsum2, &set_cookie);
...
void template_set_target_ipv6(struct TemplateSet* tmplset, ipv6address ip_them, unsigned port_them,
                              ipv6address ip_me, unsigned port_me, unsigned seqno,
 ...
 ***************************************************************************/
void template_set_target_ipv4(struct TemplateSet* tmplset, ipv4address ip_them, unsigned port_them,
                              ipv4address ip_me, unsigned port_me, unsigned seqno,
                              unsigned char* px, size_t sizeof_px, size_t* r_length)
{
    unsigned offset_ip;
    unsigned offset_tcp;
  ...
  */
    ...
    switch (tmpl->proto)
    {
        case Proto_TCP:
            ...
            xsum += (uint64_t) tmpl->ipv4.checksum_tcp + (uint64_t) ip_me + (uint64_t) ip_them +
                    (uint64_t) port_me + (uint64_t) port_them + (uint64_t) seqno;
            xsum = (xsum >> 16) + (xsum & 0xFFFF);
            xsum = (xsum >> 16) + (xsum & 0xFFFF);
            xsum = (xsum >> 16) + (xsum & 0xFFFF);
            xsum = ~xsum;
        ...
        case Proto_ICMP_timestamp:
            ...
            break;
        case Proto_VulnCheck:
            tmplset->vulncheck->set_target(tmpl, ip_them, port_them, ip_me, port_me, seqno, px,
                                           sizeof_px, r_length);
            break;
        case Proto_ARP:
            /* don't do any checksumming */
 ...
 ***************************************************************************/
static void _template_init_ipv6(struct TemplatePacket* tmpl, macaddress_t router_mac_ipv6,
 ...
 * with.
 ***************************************************************************/
static void _template_init(struct TemplatePacket* tmpl, macaddress_t source_mac,
                           macaddress_t router_mac_ipv4, macaddress_t router_mac_ipv6,
                           const void* packet_bytes, size_t packet_size, unsigned data_link_type)
{
    unsigned char* px;
    struct PreprocessedInfo parsed;
    unsigned x;
    ...
    tmpl->ipv4.checksum_ip =
        ip_header_checksum(tmpl->ipv4.packet, tmpl->ipv4.offset_ip, tmpl->ipv4.length);
    ...
    switch (parsed.ip_protocol)
    {
        ...
        case 17:                                          /* UDP */
            ...
            break;
        case 132: /* SCTP */
            tmpl->ipv4.checksum_tcp = sctp_checksum(tmpl->ipv4.packet + tmpl->ipv4.offset_tcp,
                                                    tmpl->ipv4.length - tmpl->ipv4.offset_tcp);
    ...
    if (data_link_type == PCAP_DLT_NULL /* Null VPN tunnel */)
    {
        ...
        memmove(tmpl->ipv4.packet + sizeof(int), tmpl->ipv4.packet + tmpl->ipv4.offset_ip,
                tmpl->ipv4.length);
    ...
    else if (data_link_type == PCAP_DLT_RAW /* Raw IP */)
    {
        ...
        memmove(tmpl->ipv4.packet, tmpl->ipv4.packet + tmpl->ipv4.offset_ip, tmpl->ipv4.length);
    ...
    else
    {
        ...
        LOG(0,
            "    [hint] masscan doesn't know how to format packets for this "
            "interface\n");
        exit(1);
    }
...
void template_packet_init(struct TemplateSet* templset, macaddress_t source_mac,
                          ...
                          int data_link, uint64_t entropy, const struct TemplateOptions* templ_opts)
{
    unsigned char* buf;
    size_t length;
    templset->count = 0;
    ...
    /* [SCTP] */
    _template_init(&templset->pkts[Proto_SCTP], source_mac, router_mac_ipv4, router_mac_ipv6,
                   default_sctp_template, sizeof(default_sctp_template) - 1, data_link);
    templset->count++;
    ...
    _template_init(&templset->pkts[Proto_TCP], source_mac, router_mac_ipv4, router_mac_ipv6, buf,
                   length, data_link);
    templset->count++;
    free(buf);

    /* [UDP] */
    _template_init(&templset->pkts[Proto_UDP], source_mac, router_mac_ipv4, router_mac_ipv6,
                   default_udp_template, sizeof(default_udp_template) - 1, data_link);
    templset->pkts[Proto_UDP].payloads = udp_payloads;
    templset->count++;

    /* [UDP oproto] */
    _template_init(&templset->pkts[Proto_Oproto], source_mac, router_mac_ipv4, router_mac_ipv6,
                   default_udp_template, sizeof(default_udp_template) - 1, data_link);
    templset->pkts[Proto_Oproto].payloads = oproto_payloads;
    templset->count++;

    /* [ICMP ping] */
    _template_init(&templset->pkts[Proto_ICMP_ping], source_mac, router_mac_ipv4, router_mac_ipv6,
                   default_icmp_ping_template, sizeof(default_icmp_ping_template) - 1, data_link);
    templset->count++;

    /* [ICMP timestamp] */
    _template_init(&templset->pkts[Proto_ICMP_timestamp], source_mac, router_mac_ipv4,
                   router_mac_ipv6, default_icmp_timestamp_template,
                   sizeof(default_icmp_timestamp_template) - 1, data_link);
    templset->count++;

    /* [ARP] */
    _template_init(&templset->pkts[Proto_ARP], source_mac, router_mac_ipv4, router_mac_ipv6,
                   default_arp_template, sizeof(default_arp_template) - 1, data_link);
    templset->count++;

    /* [VulnCheck] */
    if (templset->vulncheck)
    {
        _template_init(&templset->pkts[Proto_VulnCheck], source_mac, router_mac_ipv4,
                       router_mac_ipv6, templset->vulncheck->packet,
...
int template_selftest(void)
{
    ...
    template_packet_init(tmplset, macaddress_from_bytes("\x00\x11\x22\x33\x44\x55"),
