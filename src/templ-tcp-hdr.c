 ...
 */
static void safe_memmove(unsigned char* buf, size_t length, size_t to, size_t from,
                         size_t chunklength)
{
 ...
 */
static void safe_memset(unsigned char* buf, size_t length, size_t offset, int c, size_t chunklength)
{
 ...
 * the code.
 ***************************************************************************/
static void _HEXDUMP(const void* v, struct tcp_hdr_t hdr, size_t offset, const char* name)
{
 ...
 ***************************************************************************/
static unsigned inline _tcp_header_length(const unsigned char* buf, size_t offset)
{
 ...
 ***************************************************************************/
static int _consistancy_check(const unsigned char* buf, size_t length, const void* payload,
 ...
 ***************************************************************************/
static struct tcp_hdr_t _find_tcp_header(const unsigned char* buf, size_t length)
{
 ...
 *        that `len` bytes.
 ***************************************************************************/
static inline size_t _opt_next(struct tcp_hdr_t hdr, size_t offset, const unsigned char* buf)
{
...
/***************************************************************************
 ***************************************************************************/
static void _HEXDUMPopt(const unsigned char* buf, size_t length, const char* name)
{
 ...
 ***************************************************************************/
static size_t _find_opt(const unsigned char* buf, struct tcp_hdr_t hdr, unsigned in_kind,
                        unsigned* nop_count)
{
    size_t offset;
    ...
    for (offset = _opt_begin(hdr); offset < hdr.max; offset = _opt_next(hdr, offset, buf))
    {
        unsigned kind;
 ...
 ***************************************************************************/
static struct tcp_opt_t tcp_find_opt(const unsigned char* buf, size_t length, unsigned in_kind)
{
 ...
 ***************************************************************************/
static void _adjust_length(unsigned char* buf, size_t length, int adjustment, struct tcp_hdr_t hdr)
{
 ...
 ***************************************************************************/
static void _add_padding(unsigned char** inout_buf, size_t* inout_length, size_t offset,
                         unsigned pad_count)
{
    ...
    safe_memmove(buf, length, offset + pad_count, offset, (length - pad_count) - offset);
 ...
 ***************************************************************************/
static bool _normalize_padding(unsigned char** inout_buf, size_t* inout_length)
{
    unsigned char* buf = *inout_buf;
    size_t length = *inout_length;
    struct tcp_hdr_t hdr;
...

    {
        ...
        while (remove_count % 4) remove_count--;
        ...
        safe_memmove(buf, length, offset, offset + remove_count, length - (offset + remove_count));
...
/***************************************************************************
 ***************************************************************************/
static bool tcp_remove_opt(unsigned char** inout_buf, size_t* inout_length, unsigned in_kind)
{
    unsigned char* buf = *inout_buf;
    size_t length = *inout_length;
    struct tcp_hdr_t hdr;
...

    {
        ...
        while (offset + remove_length < hdr.max && buf[offset + remove_length] == 1)
            remove_length++;
...
/***************************************************************************
 ***************************************************************************/
static int _insert_field(unsigned char** inout_buf, size_t* inout_length, size_t offset_begin,
                         size_t offset_end, const unsigned char* new_data, size_t new_length)
{
    unsigned char* buf = *inout_buf;
    size_t length = *inout_length;
    int adjust = 0;
    ...
    if (adjust < 0)
    {
        safe_memmove(buf, length, offset_begin + new_length, offset_end, length - offset_end);
        length += adjust;
        buf = realloc(buf, length);
    }
 ...
 ***************************************************************************/
static size_t _squeeze_padding(unsigned char* buf, size_t length, struct tcp_hdr_t hdr,
                               unsigned in_kind)
{
    size_t offset;
    unsigned nop_count = 0;
...
/***************************************************************************
 ***************************************************************************/
static bool tcp_add_opt(unsigned char** inout_buf, size_t* inout_length, unsigned opt_kind,
                        unsigned opt_length, const unsigned char* opt_data)
{
    unsigned char* buf = *inout_buf;
    size_t length = *inout_length;
    struct tcp_hdr_t hdr;
...

    {
        ...
        else if (buf[offset] == opt_kind)
        { /* will replace old field */
        ...
        adjust = _insert_field(&buf, &length, old_begin, old_end, new_field, new_length);
        hdr.max += adjust;
    }
...
/***************************************************************************
 ***************************************************************************/
static unsigned tcp_get_mss(const unsigned char* buf, size_t length, bool* is_found)
{
...
/***************************************************************************
 ***************************************************************************/
static unsigned tcp_get_wscale(const unsigned char* buf, size_t length, bool* is_found)
{
    struct tcp_opt_t opt;
    unsigned result = 0;
...
/***************************************************************************
 ***************************************************************************/
static unsigned tcp_get_sackperm(const unsigned char* buf, size_t length, bool* is_found)
{
    struct tcp_opt_t opt;
                             ...
                             const char* new_options, size_t new_length)
{
    ...
    if (adjust > 0)
    {
        length += adjust;
        buf = realloc(buf, length);
        safe_memmove(buf, length, hdr.max + adjust, hdr.max, (length - adjust) - hdr.max);
    }
    if (adjust < 0)
    {
...
static int _selftests_run(void)
{
    ...
    for (i = 0; tests[i].pre.options; i++)
    {
        ...
        success = _replace_options(&buf, &length, tests[i].pre.options, tests[i].pre.length);
        if (!success)
