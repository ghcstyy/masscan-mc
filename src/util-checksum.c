...
static struct
{
...
} ipv6packets[] = {
    {
        ...
        "\xff\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x02", 50, 17 /* UDP */
    },
    {
        0xbf3c,
        ...
        "\xff\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x16", 88, 58 /* ICMPv6 */
    },
    {
        0x0d0e,
        ...
        "\x26\x02\xff\x52\x00\x00\x00\x6a\x00\x00\x00\x00\x1f\xd2\x94\x5a", 20, 6 /* TCP */
    },
    {0}};
...
int checksum_selftest(void)
{
    ...
    for (i = 0; ipv6packets[i].buf; i++)
    {
        sum = checksum_ipv6((const unsigned char*) ipv6packets[i].ip_src,
                            (const unsigned char*) ipv6packets[i].ip_dst, ipv6packets[i].ip_proto,
    ...
    {
        sum = checksum_ipv4(ipv4packets[i].ip_src, ipv4packets[i].ip_dst, ipv4packets[i].ip_proto,
