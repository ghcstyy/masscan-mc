...
static unsigned _parse_transform(struct BannerOutput *banout, unsigned proto,
...
static unsigned _parse_transforms(struct BannerOutput *banout, unsigned proto,
                                  struct ebuf_t ebuf, unsigned next_payload) {
...
static unsigned _parse_proposal(struct BannerOutput *banout, unsigned proto,
                                struct ebuf_t ebuf) {
  unsigned proto_id;
...
static unsigned _parse_proposals(struct BannerOutput *banout, unsigned proto,
                                 struct ebuf_t ebuf, unsigned next_payload) {
...
static unsigned _payload_security_association(struct BannerOutput *banout,
                                              unsigned proto,
                                              struct ebuf_t ebuf) {
  unsigned doi;
  unsigned bitmap;
...
static unsigned _payload_vendor_id(struct BannerOutput *banout, unsigned proto,
                                   struct ebuf_t ebuf) {
  size_t i;
  ...
  struct {
    ...
    const char *name;
      ...
      {16, "\x4a\x13\x1c\x81\x07\x03\x58\x45\x5c\x57\x28\xf2\x0e\x95\x45\x2f",
       "RFC-39947-NAT"},
      {16, "\x12\xf5\xf2\x8c\x45\x71\x68\xa9\x70\x2d\x9f\xe2\x74\xcc\x01\x00",
       "CISCO-UNITY"},
      {16, "\xaf\xca\xd7\x13\x68\xa1\xf1\xc9\x6b\x86\x96\xfc\x77\x57\x01\x00",
       "RFC3706-DPD"},
      {8,
       "\x09\x00"
       "&\x89\xdf\xd6\xb7\x12",
...
unsigned isakmp_parse(struct Output *out, time_t timestamp,
  ...
  if (resp_cookie != cookie) {
    ...
    output_report_banner(out, timestamp, ip_them, 17, port_them, PROTO_ISAKMP,
                         parsed->ip_ttl, banout_string(banout, PROTO_ISAKMP),
...
static int _test_sample(const void *sample, size_t sizeof_sample,
                        const char *expected) {
  int is_valid;
  struct BannerOutput banout[1];
  ...
  is_valid = _parse_response(banout, PROTO_ISAKMP,
                             (const unsigned char *)sample, sizeof_sample);
...

  {
    ...
    if (str_length < strlen(expected) ||
        memcmp(str, expected, strlen(expected)) != 0)
      goto fail;
  }
...
int proto_isakmp_selftest(void) {
  ...
  fail_count +=
      _test_sample(sample3, sizeof(sample3) - 1,
                   "v1.0 xchg=id-prot [KEY-XCHG] [NONCE]  {CISCO-UNITY}  "
                   "{RFC3706-DPD}   {XAUTH} [NAT-D] [NAT-D]");

  fail_count +=
      _test_sample(sample4, sizeof(sample4) - 1, "v1.0 xchg=id-prot ENCRYPTED");
