 ...
 ****************************************************************************/
static void ASN1_push(struct CertDecode* x, unsigned next_state, uint64_t remaining)
{
    static const size_t STACK_DEPTH = sizeof(x->stack.remainings) / sizeof(x->stack.remainings[0]);
    ...
    if (x->stack.depth)
    {
        ...
        x->stack.remainings[0] = (unsigned short) (x->stack.remainings[0] - remaining);
    }
    ...
    memmove(&x->stack.states[1], &x->stack.states[0], x->stack.depth * sizeof(x->stack.states[0]));
...
static unsigned ASN1_pop(struct CertDecode* x)
{
    ...
    memmove(&x->stack.remainings[0], &x->stack.remainings[1],
            x->stack.depth * sizeof(x->stack.remainings[0]));
    memmove(&x->stack.states[0], &x->stack.states[1], x->stack.depth * sizeof(x->stack.states[0]));
    return next_state;
}
 ...
 *      0 otherwise
 ****************************************************************************/
static unsigned ASN1_skip(struct CertDecode* x, unsigned* i, size_t length)
{
    unsigned len;
...
#define GOTO_ERROR(state, i, length) \
    (state) = 0xFFFFFFFF;            \
    (i) = (length);                  \
    continue
...
void spnego_decode(struct SpnegoDecode* spnego, const unsigned char* px, size_t length,
                   struct BannerOutput* banout)
{
