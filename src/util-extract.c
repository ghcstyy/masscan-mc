...
unsigned e_next_int32(struct ebuf_t* ebuf, int endian)
{
    ...
    {
        result = buf[offset + 3] << 24 | buf[offset + 2] << 16 | buf[offset + 1] << 8 |
                 buf[offset + 0] << 0;
    }
    ebuf->offset += 4;
    return result;
...
unsigned long long e_next_long64(struct ebuf_t* ebuf, int endian)
{
    ...
    if (endian == EBUF_BE)
    {
        hi = buf[offset + 0] << 24 | buf[offset + 1] << 16 | buf[offset + 2] << 8 |
             buf[offset + 3] << 0;
        lo = buf[offset + 4] << 24 | buf[offset + 5] << 16 | buf[offset + 6] << 8 |
             buf[offset + 7] << 0;
    }
    else
    {
        lo = buf[offset + 3] << 24 | buf[offset + 2] << 16 | buf[offset + 1] << 8 |
             buf[offset + 0] << 0;
        hi = buf[offset + 7] << 24 | buf[offset + 6] << 16 | buf[offset + 5] << 8 |
             buf[offset + 4] << 0;
    }
    ebuf->offset += 8;
    return hi << 32ull | lo;
