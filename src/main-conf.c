...
void masscan_usage(void) {
  ...
  printf("        read binary scan results in <filename> and save them as xml "
         "in <savefile>\n");
  exit(1);
}
...
static void masscan_echo_nic(struct Masscan *masscan, FILE *fp, unsigned i) {
  ...
  if (masscan->nic[i].src.ipv4.first != 0 ||
      masscan->nic[i].src.ipv4.last != 0) {
    ...
    if (masscan->nic[i].src.ipv4.first == masscan->nic[i].src.ipv4.last) {
      ipaddress_formatted_t fmt =
          ipv4address_fmt(masscan->nic[i].src.ipv4.first);
     ...
     */
      ...
      ipaddress_formatted_t fmt2 =
          ipv4address_fmt(masscan->nic[i].src.ipv4.last);
      fprintf(fp, "adapter-ip%s = %s-%s\n", idx_str, fmt1.string, fmt2.string);
    }
  }

  if (masscan->nic[i].src.ipv6.range) {
    if (ipv6address_is_lessthan(masscan->nic[i].src.ipv6.first,
                                masscan->nic[i].src.ipv6.last)) {
      ipaddress_formatted_t fmt1 =
          ipv6address_fmt(masscan->nic[i].src.ipv6.first);
      ipaddress_formatted_t fmt2 =
          ipv6address_fmt(masscan->nic[i].src.ipv6.last);
      fprintf(fp, "adapter-ip%s = %s-%s\n", idx_str, fmt1.string, fmt2.string);
    } else {
      ipaddress_formatted_t fmt =
          ipv6address_fmt(masscan->nic[i].src.ipv6.first);
...
static void config_top_ports(struct Masscan *masscan, unsigned maxports) {
  ...
  static const unsigned short top_tcp_ports[] = {
      ...
      26,    30,    32,    33,    37,    42,    43,    49,    70,    79,
      81,    82,    83,    84,    85,    89,    90,    99,    100,   106,
      109,   110,   113,   119,   125,   143,   144,   146,   161,   163,
      179,   199,   211,   212,   222,   254,   255,   256,   259,   264,
      280,   301,   306,   311,   340,   366,   406,   407,   416,   417,
      425,   427,   444,   458,   464,   465,   481,   497,   500,   512,
      513,   514,   515,   524,   541,   543,   544,   545,   554,   555,
      563,   593,   616,   617,   625,   631,   646,   648,   666,   667,
      668,   683,   687,   691,   700,   705,   711,   714,   720,   722,
      726,   749,   765,   777,   783,   787,   800,   801,   808,   843,
      873,   880,   888,   898,   900,   901,   902,   903,   911,   912,
      981,   987,   993,   995,   999,   1000,  1001,  1002,  1007,  1009,
      1010,  1011,  1021,  1022,  1023,  1024,  1025,  1026,  1027,  1028,
      ...
      1089,  1090,  1091,  1092,  1093,  1094,  1095,  1096,  1097,  1100,
      1102,  1104,  1105,  1106,  1107,  1108,  1110,  1111,  1112,  1113,
      1114,  1117,  1119,  1121,  1122,  1123,  1124,  1126,  1130,  1131,
      1132,  1137,  1138,  1141,  1145,  1147,  1148,  1149,  1151,  1152,
      ...
      1352,  1417,  1434,  1443,  1455,  1461,  1494,  1500,  1501,  1503,
      ...
      2522,  2557,  2601,  2602,  2604,  2605,  2607,  2608,  2638,  2701,
      2702,  2710,  2717,  2718,  2725,  2800,  2809,  2811,  2869,  2875,
      2909,  2910,  2920,  2967,  2968,  2998,  3000,  3001,  3003,  3005,
      3006,  3007,  3011,  3013,  3017,  3030,  3031,  3052,  3071,  3077,
      3128,  3168,  3211,  3221,  3260,  3261,  3268,  3269,  3283,  3300,
      3301,  3306,  3322,  3323,  3324,  3325,  3333,  3351,  3367,  3369,
      3370,  3371,  3372,  3389,  3390,  3404,  3476,  3493,  3517,  3527,
      3546,  3551,  3580,  3659,  3689,  3690,  3703,  3737,  3766,  3784,
      3800,  3801,  3809,  3814,  3826,  3827,  3828,  3851,  3869,  3871,
      3878,  3880,  3889,  3905,  3914,  3918,  3920,  3945,  3971,  3986,
      3995,  3998,  4000,  4001,  4002,  4003,  4004,  4005,  4006,  4045,
      4111,  4125,  4126,  4129,  4224,  4242,  4279,  4321,  4343,  4443,
      4444,  4445,  4446,  4449,  4550,  4567,  4662,  4848,  4899,  4900,
      4998,  5000,  5001,  5002,  5003,  5004,  5009,  5030,  5033,  5050,
      5051,  5054,  5080,  5087,  5100,  5101,  5102,  5120,  5190,  5200,
      5214,  5221,  5222,  5225,  5226,  5269,  5280,  5298,  5357,  5405,
      5414,  5431,  5432,  5440,  5500,  5510,  5544,  5550,  5555,  5560,
      5566,  5631,  5633,  5666,  5678,  5679,  5718,  5730,  5801,  5802,
      5810,  5811,  5815,  5822,  5825,  5850,  5859,  5862,  5877,  5902,
      5903,  5904,  5906,  5907,  5910,  5911,  5915,  5922,  5925,  5950,
      5952,  5959,  5960,  5961,  5962,  5963,  5987,  5988,  5989,  5998,
      5999,  6002,  6003,  6004,  6005,  6006,  6007,  6009,  6025,  6059,
      ...
      9000,  9001,  9002,  9003,  9009,  9010,  9011,  9040,  9071,  9080,
  ...
  static const unsigned max_tcp_ports =
      sizeof(top_tcp_ports) / sizeof(top_tcp_ports[0]);
  static const unsigned max_udp_ports =
      sizeof(top_udp_ports) / sizeof(top_udp_ports[0]);
...
static int SET_banners_rawudp(struct Masscan *masscan, const char *name,
                              const char *value) {
  UNUSEDPARM(name);
  if (masscan->echo) {
...
static int SET_nobanners(struct Masscan *masscan, const char *name,
                         const char *value) {
  UNUSEDPARM(name);
  if (masscan->echo) {
    return 0;
...
static int SET_output_filename(struct Masscan *masscan, const char *name,
  ...
  safe_strcpy(masscan->output.filename, sizeof(masscan->output.filename),
              value);
  return CONF_OK;
}
...
static int SET_script(struct Masscan *masscan, const char *name,
  ...
  UNUSEDPARM(name);
  if (masscan->echo) {
    if ((masscan->scripting.name && masscan->scripting.name[0]) ||
        masscan->echo_all)
...
static int SET_output_stylesheet(struct Masscan *masscan, const char *name,
  ...
  safe_strcpy(masscan->output.stylesheet, sizeof(masscan->output.stylesheet),
              value);
  return CONF_OK;
}

static int SET_topports(struct Masscan *masscan, const char *name,
                        const char *value) {
...
static int SET_tcp_mss(struct Masscan *masscan, const char *name,
                       const char *value) {
  /* h/t @IvreRocks */
...
static int SET_tcp_wscale(struct Masscan *masscan, const char *name,
                          const char *value) {
...
static int SET_tcp_tsecho(struct Masscan *masscan, const char *name,
                          const char *value) {
...
static int SET_tcp_sackok(struct Masscan *masscan, const char *name,
                          const char *value) {
  if (masscan->echo) {
    if (masscan->templ_opts) {
...
static int SET_debug_tcp(struct Masscan *masscan, const char *name,
                         const char *value) {
...
struct ConfigParameter config_parameters[] = {
    ...
    {"rotate",
     SET_rotate_time,
     0,
     {"output-rotate", "rotate-output", "rotate-time", 0}},
    {"rotate-dir",
     SET_rotate_directory,
     0,
     {"output-rotate-dir", "rotate-directory", 0}},
    {"rotate-offset", SET_rotate_offset, 0, {"output-rotate-offset", 0}},
    {"rotate-size",
     SET_rotate_filesize,
     0,
     {"output-rotate-filesize", "rotate-filesize", 0}},
...
void masscan_set_parameter(struct Masscan *masscan, const char *name,
  ...
  if (EQUALS("conf", name) || EQUALS("config", name)) {
  ...
  } else if (EQUALS("adapter", name) || EQUALS("if", name) ||
    ...
    snprintf(masscan->nic[index].ifname, sizeof(masscan->nic[index].ifname),
             "%s", value);
    ...
    default:
      ...
      LOG(0, "hint   addresses look like \"192.168.1.23\" or "
             "\"2001:db8:1::1ce9\".\n");
      exit(1);
    }
    ...
    if (masscan->nic[index].my_mac_count != 0) {
      ipaddress_formatted_t fmt1 =
          macaddress_fmt(masscan->nic[index].source_mac);
    ...
    if (masscan->op == 0)
      masscan->op = Operation_Scan;
             ...
             EQUALS("destination-port", name) || EQUALS("target-port", name)) {
    ...
    fprintf(stderr, "nmap(%s): unsupported: we randomize all the groups!\n",
            name);
    exit(1);
  } else if (EQUALS("min-parallelism", name) ||
             EQUALS("max-parallelism", name)) {
    fprintf(stderr, "nmap(%s): unsupported: we all the parallel!\n", name);
    exit(1);
  } else if (EQUALS("min-rtt-timeout", name) ||
             EQUALS("max-rtt-timeout", name) ||
...
void masscan_echo(struct Masscan *masscan, FILE *fp, unsigned is_echo_all) {
  ...
  for (i = 0; i < masscan->targets.ports.count; i++) {
    ...
    do {
      ...
      } else if (Templ_Oproto_first <= rrange.begin &&
                 rrange.begin <= Templ_Oproto_last) {
 ...
 *  10.0.0.128/25
 ***************************************************************************/
void masscan_echo_cidr(struct Masscan *masscan, FILE *fp,
                       unsigned is_echo_all) {
  unsigned i;
