...
/***************************************************************************
 ***************************************************************************/
static const char* cndb_lookup(unsigned ip)
{
    ...
    while (entry && entry->ip != ip) entry = entry->next;
    if (entry)
        return entry->name;
    else
    {
...
/***************************************************************************
 ***************************************************************************/
static void cndb_add(unsigned ip, const unsigned char* name, size_t name_length)
{
...
static void print_counts()
{
    ...
    const char* count_names[] = {
        "Unknown", "NAS",  "WiFi",  "FW", "X509",     "Conf", "VM", "Cam", "VPN", "PBX", "Printer",
        "default", "mail", "admin", "AV", "honeypot", "box",  0,    "",    "",    ""};
...
static void xname_init(void)
{
    ...
    for (i = 0; mynames[i].name; i++)
    {
        ...
        smack_add_pattern(global_xnames, pattern, len, id, 0);
    }
...
/***************************************************************************
 ***************************************************************************/
static unsigned found_type(const unsigned char* banner, size_t banner_length)
{
    size_t id;
    unsigned state = 0;
    unsigned offset = 0;
    ...
    id = smack_search_next(global_xnames, &state, banner, &offset, (unsigned) banner_length);
...
void readscan_report(unsigned ip, unsigned app_proto, unsigned char** r_data, size_t* r_data_length)
{
...
void readscan_report_init(void)
{
    if (global_xnames == NULL)
        xname_init();
}

void readscan_report_print(void)
{
    print_counts();
}
