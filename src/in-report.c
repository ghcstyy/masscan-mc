...
/***************************************************************************
 ***************************************************************************/
static void cndb_add(unsigned ip, const unsigned char *name,
                     size_t name_length) {
...
static void print_counts() {
  ...
  const char *count_names[] = {
      "Unknown", "NAS",      "WiFi", "FW",      "X509",    "Conf", "VM",
      "Cam",     "VPN",      "PBX",  "Printer", "default", "mail", "admin",
      "AV",      "honeypot", "box",  0,         "",        "",     ""};
...
static unsigned found_type(const unsigned char *banner, size_t banner_length) {
  ...
  id = smack_search_next(global_xnames, &state, banner, &offset,
...
void readscan_report(unsigned ip, unsigned app_proto, unsigned char **r_data,
  ...
  if (app_proto == PROTO_X509_CERT) {
    ...
    /* kludge */
    if (data_length == 31 &&
        memcmp(data, "SSL[heartbeat] SSL[HEARTBLEED] ", 31) == 0)
      return;
  }
}
...
void readscan_report_print(void) { print_counts(); }
