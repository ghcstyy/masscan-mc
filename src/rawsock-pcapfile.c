 ...
 */
static unsigned smells_like_valid_packet(const unsigned char* px, unsigned length,
                                         unsigned byte_order, unsigned link_type)
{
 ...
 */
int pcapfile_readframe(struct PcapFile* capfile, unsigned* r_time_secs, unsigned* r_time_usecs,
                       unsigned* r_original_length, unsigned* r_captured_length, unsigned char* buf,
                       unsigned sizeof_buf)
{
    size_t bytes_read;
    ...
    *r_original_length = PCAP32(byte_order, header + 12);
    ...
    while (is_corrupt)
    {
        ...
        for (i = 0; i < bytes_read; i++)
        {
            ...
            if (!smells_like_valid_packet(tmp + i, (unsigned) (bytes_read - i), byte_order,
                                          capfile->linktype))
                continue;
            ...
            return pcapfile_readframe(capfile, r_time_secs, r_time_usecs, r_original_length,
                                      r_captured_length, buf, sizeof_buf);
        }
...
void pcapfile_writeframe(struct PcapFile* capfile, const void* buffer, unsigned buffer_size,
                         unsigned original_length, unsigned time_sec, unsigned time_usec)
{
    unsigned char header[16];
