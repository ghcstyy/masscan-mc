  ...
  *r_offset = offset;
  ...
  for (i = 0; name_to_types[i].name; i++) {
    if (name_length != name_to_types[i].length)
      continue;
    if (strncasecmp(line + name_offset, name_to_types[i].name, name_length) ==
        0) {
      break;
    }
  }
...
static unsigned parse_number(struct NmapServiceProbeList *list,
  ...
  if (offset != line_length) {
    fprintf(stderr, "%s:%u:%u: unexpected character '%c'\n", list->filename,
            list->line_number, (unsigned)offset,
...
static void parse_line(struct NmapServiceProbeList *list, const char *line) {
  ...
  case SvcP_Exclude:
    if (list->count) {
       ...
       * before any Probes */
      fprintf(stderr,
              "%s:%u:%u: 'Exclude' directive only valid before any 'Probe'\n",
              filename, line_number, (unsigned)offset);
    } else {
...
/*****************************************************************************
 *****************************************************************************/
void nmapserviceprobes_print(const struct NmapServiceProbeList *list,
                             FILE *fp) {
  unsigned i;
  if (list == NULL)
    return;
  ...
  for (i = 0; i < list->count; i++) {
    ...
    fprintf(fp, "Probe %s %s q", (probe->protocol == 6) ? "TCP" : "UDP",
            probe->name);
