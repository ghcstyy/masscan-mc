 ...
 *****************************************************************************/
static enum SvcP_RecordType parse_type(const char* line, size_t* r_offset, size_t line_length)
{
    static const struct
    {
        const char* name;
    ...
    while (offset < line_length && isspace(line[offset])) offset++; /* trim whitespace after name */
    *r_offset = offset;
...
/*****************************************************************************
 *****************************************************************************/
static struct RangeList parse_ports(struct NmapServiceProbeList* list, const char* line,
...
/*****************************************************************************
 *****************************************************************************/
static unsigned parse_number(struct NmapServiceProbeList* list, const char* line, size_t offset,
                             size_t line_length)
{
    /* Examples:
     totalwaitms 6000
    ...
    if (offset != line_length)
    {
        fprintf(stderr, "%s:%u:%u: unexpected character '%c'\n", list->filename, list->line_number,
                (unsigned) offset, isprint(line[offset]) ? line[offset] : '.');
    }
...
/*****************************************************************************
 *****************************************************************************/
static char* parse_name(const char* line, size_t* r_offset, size_t line_length)
{
    ...
    while (*r_offset < line_length && !isspace(line[*r_offset])) (*r_offset)++;
...
/*****************************************************************************
 *****************************************************************************/
static struct ServiceProbeFallback* parse_fallback(struct NmapServiceProbeList* list,
                                                   ...
                                                   size_t line_length)
{
    ...
    while (offset < line_length)
    {
        ...
        /* grab all characters until first space */
        name_offset = offset;
        while (offset < line_length && !isspace(line[offset]) && line[offset] != ',') offset++;
        name_length = offset - name_offset;
...
/*****************************************************************************
 *****************************************************************************/
static void parse_probe(struct NmapServiceProbeList* list, const char* line, size_t offset,
                        size_t line_length)
{
    /* Examples:
     ...
     */
    {
        ...
        if (line[offset++] != 'q')
        {
            fprintf(stderr, "%s:%u:%u: expected 'q', found '%c'\n", filename, line_number,
                    (unsigned) offset, isprint(line[offset - 1]) ? line[offset - 1] : '.');
            goto parse_error;
        }
        ...
        while (offset < line_length && line[offset] != delimiter)
        {
            ...
            {
                default:
                    fprintf(stderr, "%s:%u: %.*s\n", filename, line_number, (unsigned) line_length,
                            line);
                    fprintf(stderr, "%s:%u:%u: unexpected escape character '%c'\n", filename,
                            line_number, (unsigned) offset - 1,
                ...
                case 'x':
                    ...
                    if (offset + 2 >= line_length || line[offset + 0] == delimiter ||
                    ...
                    x[x_offset++] =
                        (char) (hexval(line[offset + 0]) << 4 | hexval(line[offset + 1]));
                    offset += 2;
                    break;
            }
        }
        ...
        if (offset >= line_length || line[offset] != delimiter)
        {
            fprintf(stderr, "%s:%u:%u: missing end delimiter '%c'\n", filename, line_number,
                    (unsigned) offset, isprint(delimiter) ? delimiter : '.');
            goto parse_error;
        }
        // offset++;
    }
...
/*****************************************************************************
 *****************************************************************************/
static struct ServiceProbeMatch* parse_match(struct NmapServiceProbeList* list, const char* line,
                                             size_t offset, size_t line_length)
{
    /* Examples:
     ...
     m|@ABCDEFGHIJKLMNOPQRSTUVWXYZ| match uucp m|^login: login: login: $| p/NetBSD
     uucpd/ o/NetBSD/ cpe:/o:netbsd:netbsd/a match printer m|^([\w-_.]+): lpd:
     ...
     */
    {
        ...
        regex_offset = offset;
        while (offset < line_length && line[offset] != delimiter) offset++;
    ...
    while (offset < line_length)
    {
        ...
        if (offset + 2 >= line_length)
        {
            fprintf(stderr, "%s:%u:%u: unexpected character at end of line '%c'\n", filename,
                    line_number, (unsigned) offset, isprint(line[offset]) ? line[offset] : '.');
            goto parse_error;
        }
        ...
        if (offset < line_length && !isspace(line[offset]))
        {
            fprintf(stderr, "%s:%u:%u: unexpected character after delimiter '%c'\n", filename,
                    line_number, (unsigned) offset, isprint(delimiter) ? delimiter : '.');
            goto parse_error;
        }
        while (offset < line_length && isspace(line[offset])) offset++;
...
static void parse_line(struct NmapServiceProbeList* list, const char* line)
{
    ...
    while (offset && isspace(line[offset])) offset++;
    while (line_length && isspace(line[line_length - 1])) line_length--;
    ...
    switch ((int) type)
    {
        case SvcP_Unknown:
            fprintf(stderr, "%s:%u:%u: unknown type: '%.*s'\n", filename, line_number,
                    (unsigned) offset, (int) offset - 0, line);
            return;
        case SvcP_Exclude:
            if (list->count)
            {
            ...
            return;
        case SvcP_Probe:
            /* Creates a new probe record, all the other types (except 'Exclude')
             * operate on the current probe record */
...
/*****************************************************************************
 *****************************************************************************/
static struct NmapServiceProbeList* nmapserviceprobes_new(const char* filename)
{
...
/*****************************************************************************
 *****************************************************************************/
struct NmapServiceProbeList* nmapserviceprobes_read_file(const char* filename)
{
    FILE* fp;
    char line[32768];
    struct NmapServiceProbeList* result;
...
/*****************************************************************************
 *****************************************************************************/
static void nmapserviceprobes_print_ports(const struct RangeList* ranges, FILE* fp,
                                          const char* prefix, int default_proto)
{
    unsigned i;
        ...
        if (default_proto != proto)
        {
            ...
            switch (proto)
            {
                ...
                case Templ_SCTP:
                    fprintf(fp, "S");
                    break;
                case Templ_ICMP_echo:
                    fprintf(fp, "e");
                    break;
                case Templ_ICMP_timestamp:
                    fprintf(fp, "t");
                    break;
                case Templ_ARP:
                    fprintf(fp, "A");
                    break;
                case Templ_VulnCheck:
                    fprintf(fp, "v");
                    break;
            }
        }
...
/*****************************************************************************
 *****************************************************************************/
static void nmapserviceprobes_print_dstring(FILE* fp, const char* string, size_t length,
                                            int delimiter)
{
    size_t i;
...
/*****************************************************************************
 *****************************************************************************/
static void nmapserviceprobes_print_hello(FILE* fp, const char* string, size_t length,
                                          int delimiter)
{
    size_t i;
...
/*****************************************************************************
 *****************************************************************************/
void nmapserviceprobes_print(const struct NmapServiceProbeList* list, FILE* fp)
{
    unsigned i;
    if (list == NULL)
        return;
    ...
    for (i = 0; i < list->count; i++)
    {
        ...
        fprintf(fp, "Probe %s %s q", (probe->protocol == 6) ? "TCP" : "UDP", probe->name);
        ...
        nmapserviceprobes_print_ports(&probe->ports, fp, "ports",
                                      (probe->protocol == 6) ? Templ_TCP : Templ_UDP);
...
/*****************************************************************************
 *****************************************************************************/
void nmapserviceprobes_free(struct NmapServiceProbeList* list)
{
    unsigned i;
...
/*****************************************************************************
 *****************************************************************************/
int nmapserviceprobes_selftest(void)
{
    const char* lines[] = {
        "Exclude 53,T:9100,U:30000-40000\n",
        "Probe UDP DNSStatusRequest q|\\0\\0\\x10\\0\\0\\0\\0\\0\\0\\0\\0\\0|\n",
        ...
        "h/$1/\n",
        "match afs m|^[\\d\\D]{28}\\s*(OpenAFS)([\\d\\.]{3}[^\\s\\0]*)\\0| p/$1/ "
        "v/$2/\n",
        0};
    unsigned i;
