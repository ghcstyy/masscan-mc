...
const unsigned char zeroaccess_getL[] = {
    0x46, 0x5d, 0x49, 0x9e, 0x28, 0x94, 0x8d, 0xab, 0xc9, 0xc0, 0xd1, 0x99, 0xe0, 0xf2, 0xc2, 0x5e,
};
 ...
 * decryption.
 ***************************************************************************/
static unsigned zadecrypt(const unsigned char* src, size_t src_len, unsigned char* dst,
                          size_t dst_len)
{
    unsigned key;
    size_t i;
 ...
 ***************************************************************************/
static unsigned generate_getL(unsigned char* out_buf, size_t out_buf_len, unsigned xrand)
{
                           ...
                           unsigned length, struct PreprocessedInfo* parsed, uint64_t entropy)
{
    ...
    buf[0] = '\0';
    len = zadecrypt(px + parsed->app_offset, parsed->app_length, buf, sizeof(buf));
...

    {
        ...
        for (i = 0; i < ip_count; i++)
        {
            ...
            snprintf(szaddr, sizeof(szaddr), "%u.%u.%u.%u ", (unsigned char) (ip_found >> 24),
                     (unsigned char) (ip_found >> 16), (unsigned char) (ip_found >> 8),
                     (unsigned char) (ip_found >> 0));
            banout_append(banout, PROTO_UDP_ZEROACCESS, szaddr, strlen(szaddr));
        }
    }

    output_report_banner(out, timestamp, ip_them, 17, port_them, PROTO_UDP_ZEROACCESS,
                         parsed->ip_ttl, banout_string(banout, PROTO_UDP_ZEROACCESS),
...
/***************************************************************************
 ***************************************************************************/
static const unsigned char sample[] = {0xda, 0xbe, 0x6e, 0xce, 0x28, 0x94, 0x8d, 0xab,
                                       0xc9, 0xc0, 0xd1, 0x99, 0xec, 0xd6, 0xa9, 0x3c};
