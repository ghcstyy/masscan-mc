...
/***************************************************************************
 ***************************************************************************/
static struct massip_parser* _parser_init(struct massip_parser* p)
{
    memset(p, 0, sizeof(*p));
    p->line_number = 1;
...
/***************************************************************************
 ***************************************************************************/
static void _parser_destroy(struct massip_parser* p)
{
    UNUSEDPARM(p);
}
...
/***************************************************************************
 ***************************************************************************/
static void _parser_err(struct massip_parser* p, unsigned long long* line_number,
 ...
 * address
 */
static void _init_next_address(struct massip_parser* p, int is_second)
{
    p->tmp = 0;
    p->ipv6.ellision_index = 8;
    p->ipv6.index = 0;
...
static unsigned _parser_finish_ipv6(struct massip_parser* p)
{
    ...
    /* Handle ellision */
    memmove(&p->ipv6.tmp[8 - (index - ellision)], &p->ipv6.tmp[ellision],
            sizeof(p->ipv6.tmp[0]) * (index - ellision));
    memset(&p->ipv6.tmp[ellision], 0, sizeof(p->ipv6.tmp[0]) * (8 - index));

    /* Copy over to begin/end. We parse the address as a series of 16-bit
     * integers, but return the result as two 64-bit integers */
    {
        ipv6address a;
        a.hi = (uint64_t) p->ipv6.tmp[0] << 48ULL | (uint64_t) p->ipv6.tmp[1] << 32ULL |
               (uint64_t) p->ipv6.tmp[2] << 16ULL | (uint64_t) p->ipv6.tmp[3] << 0ULL;
        a.lo = (uint64_t) p->ipv6.tmp[4] << 48ULL | (uint64_t) p->ipv6.tmp[5] << 32ULL |
               (uint64_t) p->ipv6.tmp[6] << 16ULL | (uint64_t) p->ipv6.tmp[7] << 0ULL;
        if (p->ipv6.is_second)
            p->ipv6._end = a;
        else
        {
 ...
 ***************************************************************************/
static void _parser_get_ipv6(struct massip_parser* state, ipv6address* begin, ipv6address* end)
{
 ...
 ***************************************************************************/
static int _switch_to_ipv6(struct massip_parser* p, int old_state)
{
    unsigned num = p->tmp;
...
enum
{
    ...
    IPV4_nnn_nnn_nnn_n,
    IPV4_nnn_nnn_nnn_nn,
    IPV4_nnn_nnn_nnn_nnn,
    IPV4_nnn_nnn_nnn_nnn_,
    IPV4e_n,
    IPV4e_nn,
    IPV4e_nnn,
    IPV4e_nnn_,
    ...
    IPV4e_nnn_nnn_nnn_,
    IPV4e_nnn_nnn_nnn_n,
    IPV4e_nnn_nnn_nnn_nn,
    IPV4e_nnn_nnn_nnn_nnn,
    IPV4e_nnn_nnn_nnn_nnn_,
 ...
 */
static void _ipv4_apply_cidr(unsigned* begin, unsigned* end, unsigned bitcount)
{
...
/**
 * Given an address 'being' and a 'prefix', return the 'begin' and 'end' address
 * of the range.
 * @param begin
 *      An in/out parameter. This may have some extra bits somewhere in the
 * range. These will be masked off and set to zero when the function returns.
 * @param end
 *      An out parameter. This will be set to the last address of the range,
 * meaning that all the trailing bits will be set to '1'.
 * @parame prefix
 ...
 */
static void _ipv6_apply_cidr(ipv6address* begin, ipv6address* end, unsigned prefix)
{
    ipv6address mask;
 ...
 ***************************************************************************/
static int rangefile_test_error(const char* buf, unsigned long long in_line_number,
                                unsigned long long in_char_number, unsigned which_test)
{
    ...
    if (!is_error)
    {
        ...
        switch (err)
        {
            ...
            default:
                _parser_err(p, &line_number, &char_number);
                fprintf(stderr, "[-] %s:%llu:%llu: invalid IP address on line #%llu\n", filename,
                        line_number, char_number, line_number);
                is_error = true;
                break;
            case Found_IPv4:
                rangelist_add_range(targets_ipv4, begin, end);
...
ipv6address massip_parse_ipv6(const char* line)
{
    struct massip_parser p[1];
...
unsigned massip_parse_ipv4(const char* line)
{
    struct massip_parser p[1];
    size_t count = strlen(line);
    size_t offset = 0;
...
enum RangeParseResult massip_parse_range(const char* line, size_t* offset, size_t count,
 ...
 * commas and spaces.
 */
static int selftest_massip_parse_range(void)
{
    struct testcases
    {
        const char* line;
    ...
    } cases[] = {{"0.0.1.0/24,0.0.3.0-0.0.4.0", {{{0x100, 0x1ff}}, {{0x300, 0x400}}}},
        ...
        while (offset < length)
        {
            ...
            switch (x)
            {
                ...
                case Ipv4_Address:
                    ...
                    {
                        fprintf(
                            stdout, "[-] %u.%u.%u.%u - %u.%u.%u.%u\n",
                            (unsigned char) (range4.begin >> 24),
                            (unsigned char) (range4.begin >> 16),
                            (unsigned char) (range4.begin >> 8),
                            (unsigned char) (range4.begin >> 0), (unsigned char) (range4.end >> 24),
                            (unsigned char) (range4.end >> 16), (unsigned char) (range4.end >> 8),
...
/***************************************************************************
 ***************************************************************************/
static int rangefile6_test_buffer(struct massip_parser* parser, const char* buf,
                                  ipv6address expected_begin, ipv6address expected_end)
{
    size_t length = strlen(buf);
    size_t offset = 0;
...
struct
{
    ...
    {"240e:33c:2:c080:d08:d0e:b53:e74e",
     {0x240e033c0002c080ULL, 0x0d080d0e0b53e74eULL},
     {0x240e033c0002c080ULL, 0x0d080d0e0b53e74eULL}},
    ...
    {"2c0f:ff00:0:a:face:b00c:0:a7",
     {0x2c0fff000000000aULL, 0xfaceb00c000000a7ULL},
     {0x2c0fff000000000aULL, 0xfaceb00c000000a7ULL}},
    {"2a01:5b40:0:4a01:0:e21d:789f:59b1",
     {0x2a015b4000004a01ULL, 0x0000e21d789f59b1ULL},
     {0x2a015b4000004a01ULL, 0x0000e21d789f59b1ULL}},
 ...
 ***************************************************************************/
int massip_parse_selftest(void)
{
    int x = 0;
    size_t i;
    ...
    {
        x += rangefile6_test_buffer(parser, test_cases[i].string, test_cases[i].begin,
                                    test_cases[i].end);
        if (x)
        {
