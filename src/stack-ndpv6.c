...
static inline void _append_bytes(unsigned char* buf, size_t* r_offset, size_t max,
                                 const void* v_bytes, size_t len)
{
...
static inline void _append_short(unsigned char* buf, size_t* offset, size_t max, unsigned num)
{
    if (2 > max - *offset)
    {
        *offset = max;
...
static inline unsigned _read_byte(const unsigned char* buf, size_t* offset, size_t max)
{
    if (*offset + 1 < max)
    {
        return buf[(*offset)++];
...
static inline unsigned _read_short(const unsigned char* buf, size_t* offset, size_t max)
{
    if (*offset + 2 <= max)
    {
        unsigned result;
...
static inline unsigned _read_number(const unsigned char* buf, size_t* offset, size_t max)
{
    if (*offset + 4 <= max)
    {
        unsigned result;
...
static inline ipv6address_t _read_ipv6(const unsigned char* buf, size_t* offset, size_t max)
{
 ...
 */
int stack_ndpv6_incoming_request(struct stack_t* stack, struct PreprocessedInfo* parsed,
                                 const unsigned char* px, size_t length)
{
    ...
    xsum = checksum_ipv6(buf2 + offset_ip_src, buf2 + offset_ip_dst, 58, offset - offset_icmpv6,
                         buf2 + offset_icmpv6);
...
static int _extract_router_advertisement(const unsigned char* buf, size_t length,
                                         struct PreprocessedInfo* parsed, ipv6address my_ipv6,
                                         ipv6address* router_ip, macaddress_t* router_mac)
{
    size_t offset;
...
/****************************************************************************
 ****************************************************************************/
int stack_ndpv6_resolve(struct Adapter* adapter, ipv6address my_ipv6, macaddress_t my_mac_address,
                        macaddress_t* router_mac)
{
    ...
    xsum = checksum_ipv6(buf + offset_ip_src, buf + offset_ip_dst, 58, offset - offset_icmpv6,
                         buf + offset_icmpv6);
    ...
    for (;;)
    {
        ...
        err =
            _extract_router_advertisement(buf2, length2, &parsed, my_ipv6, &router_ip, router_mac);
        if (err)
            continue;
