  ...
  for (i = 0; tests[i].in.end != 0; i++) {
    ...
    if (out.begin != tests[i].out.begin || out.end != tests[i].out.end ||
      ...
      fprintf(stderr, "[%u] 0x%08x->0x%08x  /%u   0x%08x->0x%08x /%u\n",
              (unsigned)i, out.begin, out.end, prefix_bits, tests[i].out.begin,
              tests[i].out.end, tests[i].prefix_bits);
      return 1;
    }
  }
...
/***************************************************************************
 ***************************************************************************/
int range_is_valid(struct Range range) { return range.begin <= range.end; }
...
uint64_t rangelist_count(const struct RangeList *targets) {
  ...
  for (i = 0; i < targets->count; i++) {
    result +=
        (uint64_t)targets->list[i].end - (uint64_t)targets->list[i].begin + 1UL;
  }
