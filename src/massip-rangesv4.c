...
#define REGRESS(x) \
    if (!(x))      \
    return (fprintf(stderr, "regression failed %s:%d\n", __FILE__, __LINE__) | 1)
 ...
 * as a linear search.
 ***************************************************************************/
int rangelist_is_contains(const struct RangeList* targets, unsigned addr)
{
    unsigned i;
    ...
    for (i = 0; tests[i].in.end != 0; i++)
    {
            ...
            prefix_bits != tests[i].prefix_bits)
        {
            fprintf(stderr, "[%u] 0x%08x->0x%08x  /%u   0x%08x->0x%08x /%u\n", (unsigned) i,
                    out.begin, out.end, prefix_bits, tests[i].out.begin, tests[i].out.end,
                    tests[i].prefix_bits);
            return 1;
        }
    }
 ...
 ***************************************************************************/
void rangelist_add_range(struct RangeList* targets, unsigned begin, unsigned end)
{
    struct Range range;
 ...
 * ports are stored */
void rangelist_add_range_tcp(struct RangeList* targets, unsigned begin, unsigned end)
{
    rangelist_add_range(targets, Templ_TCP + begin, Templ_TCP + end);
}

/** Use this when adding UDP ports, to avoid the comoplication of how
 * ports are stored */
void rangelist_add_range_udp(struct RangeList* targets, unsigned begin, unsigned end)
{
    rangelist_add_range(targets, Templ_UDP + begin, Templ_UDP + end);
}
 ...
 * its replacement.
 ***************************************************************************/
static void rangelist_remove_range(struct RangeList* targets, unsigned begin, unsigned end)
{
    unsigned i;
    struct Range x;
...
static void rangelist_add_range2(struct RangeList* targets, struct Range range)
{
    rangelist_add_range(targets, range.begin, range.end);
}
static void rangelist_remove_range2(struct RangeList* targets, struct Range range)
{
 ...
 ***************************************************************************/
static int parse_ipv4(const char* line, unsigned* inout_offset, unsigned max, unsigned* ipv4)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        /* clear leading zeros */
        while (offset < max && line[offset] == '0') offset++;
...
struct Range range_parse_ipv4(const char* line, unsigned* inout_offset, unsigned max)
{
    ...
    /* trim whitespace */
    while (offset < max && isspace(line[offset] & 0xFF)) offset++;
    ...
    if (offset < max && line[offset] == '-')
    {
        ...
        if (ip < result.begin)
        {
            ...
            LOG(0,
                "err: ending addr %u.%u.%u.%u cannot come before starting addr "
                "%u.%u.%u.%u\n",
                ((ip >> 24) & 0xFF), ((ip >> 16) & 0xFF), ((ip >> 8) & 0xFF), ((ip >> 0) & 0xFF),
                ((result.begin >> 24) & 0xFF), ((result.begin >> 16) & 0xFF),
                ((result.begin >> 8) & 0xFF), ((result.begin >> 0) & 0xFF));
        }
        else
            result.end = ip;
        goto end;
 ...
 ***************************************************************************/
static void rangelist_exclude2(struct RangeList* targets, const struct RangeList* excludes)
{
    unsigned i;
 ...
 *    the high addresses.
 */
static void range_apply_exclude(const struct Range exclude, struct Range* target,
                                struct Range* split)
{
...
/***************************************************************************
 ***************************************************************************/
int range_is_valid(struct Range range)
{
...
void rangelist_exclude(struct RangeList* targets, struct RangeList* excludes)
{
    ...
    for (i = 0; i < targets->count; i++)
    {
        ...
        while (x < excludes->count && excludes->list[x].end < range.begin) x++;
 ...
 ***************************************************************************/
static unsigned rangelist_pick_linearsearch(const struct RangeList* targets, uint64_t index)
{
    unsigned i;
...
static int regress_pick2()
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        REGRESS(memcmp(targets->list, duplicate->list, targets->count * sizeof(targets->list[0])) ==
                0);
 ...
 ***************************************************************************/
const char* rangelist_parse_ports(struct RangeList* ports, const char* string, unsigned* is_error,
 ...
 ***************************************************************************/
static bool rangelist_is_equal(const struct RangeList* lhs, const struct RangeList* rhs)
{
    unsigned i;
