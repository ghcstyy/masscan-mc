...
/*
 ...
 +--------+--------+--------+--------+ NTLMSSP_NEGOTIATE_TARGET_INFO  flag not
 set |           TargetInfoOffset        |
 +--------+--------+--------+--------+
 ...
 TargetNameBufferOffset (4 bytes):
    ...
    bytes, from the beginning of the CHALLENGE_MESSAGE to TargetName in Payload.
 ...
 VERSION FIELDS:
  ...
  ProductBuild (2 bytes):
    ...
    system in use. This field SHOULD be set to a 16-bit quantity that identifies
 the operating system build number." NTLMRevisionCurrent (1 byte): "An 8-bit
...
static void append_unicode_string(struct BannerOutput* banout, unsigned proto, const char* name,
...
void ntlmssp_decode(struct NtlmsspDecode* x, const unsigned char* px, size_t length,
                    struct BannerOutput* banout)
{
    ...
    {
        char buf[64];
        snprintf(buf, sizeof(buf), " version=%u.%u.%u ntlm-ver=%u", px[48], px[49],
                 px[50] | px[51] << 8, px[55]);
...
void ntlmssp_decode_init(struct NtlmsspDecode* x, size_t length)
{
    memset(x, 0, sizeof(*x));
