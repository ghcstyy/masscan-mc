...
#define TCP_SEQNO(px, i)                                                       \
  (px[i + 4] << 24 | px[i + 5] << 16 | px[i + 6] << 8 | px[i + 7])
#define TCP_ACKNO(px, i)                                                       \
  (px[i + 8] << 24 | px[i + 9] << 16 | px[i + 10] << 8 | px[i + 11])
...
enum TCB_result stack_incoming_tcp(
    struct TCP_ConnectionTable *tcpcon, struct TCP_Control_Block *entry,
 ...
 */
struct TCP_Control_Block *
tcpcon_create_tcb(struct TCP_ConnectionTable *tcpcon, ipaddress ip_src,
