...
/****************************************************************************
 ****************************************************************************/
static const char* response_code(unsigned code)
{
 ...
 ****************************************************************************/
static bool is_attr_char(unsigned c)
{
    switch (c)
    {
        case '!':
...
/****************************************************************************
 ****************************************************************************/
static struct CoapLink* parse_links(const unsigned char* px, unsigned offset, unsigned length,
                                    size_t* r_count)
{
    struct CoapLink* l;
    ...
    for (; offset < length; offset++) switch (state)
        {
            case INVALID:
                offset = length;
                break;
            case LINK_BEGIN:
                ...
                links[count].link_offset =
                    length; /* indicate end-of-list by pointing to end-of-input */
...
/****************************************************************************
 ****************************************************************************/
static bool coap_parse(const unsigned char* px, size_t length, struct BannerOutput* banout,
                       unsigned* request_id)
{
    unsigned version;
    unsigned type;
    ...
    *request_id = px[2] << 8 | px[3];
    ...
    while (offset < length)
    {
        ...
        switch (optnum)
        {
            case 0xFFFFFFFF:
                break;
            case 1:
                banout_append(banout, PROTO_COAP, " /If-Match/", AUTO_LEN);
                break;
            case 3:
                banout_append(banout, PROTO_COAP, " /Uri-Host/", AUTO_LEN);
                break;
            case 4:
                banout_append(banout, PROTO_COAP, " /Etag", AUTO_LEN);
                break;
            case 5:
                banout_append(banout, PROTO_COAP, " /If-None-Match/", AUTO_LEN);
                break;
            case 7:
                banout_append(banout, PROTO_COAP, " /Uri-Port/", AUTO_LEN);
                break;
            case 8:
                banout_append(banout, PROTO_COAP, " /Location-Path/", AUTO_LEN);
                break;
            case 11:
                banout_append(banout, PROTO_COAP, " /Uri-Path/", AUTO_LEN);
                break;
            case 12:
                ...
                break;
            case 14:
                banout_append(banout, PROTO_COAP, " /Max-Age/", AUTO_LEN);
                break;
            case 15:
                banout_append(banout, PROTO_COAP, " /Uri-Query/", AUTO_LEN);
                break;
            case 17:
                banout_append(banout, PROTO_COAP, " /Accept/", AUTO_LEN);
                break;
            case 20:
                banout_append(banout, PROTO_COAP, " /Location-Query/", AUTO_LEN);
                break;
            case 35:
                banout_append(banout, PROTO_COAP, " /Proxy-Uri/", AUTO_LEN);
                break;
            case 39:
                banout_append(banout, PROTO_COAP, " /Proxy-Scheme/", AUTO_LEN);
                break;
            case 60:
                banout_append(banout, PROTO_COAP, " /Size1/", AUTO_LEN);
                break;
            default:
                banout_append(banout, PROTO_COAP, " /(Unknown)/", AUTO_LEN);
                break;
        }
    ...
    {
        case 0:
            banout_append(banout, PROTO_COAP, " text-plain", AUTO_LEN);
            break;
        case 40:
            banout_append(banout, PROTO_COAP, " application/link-format", AUTO_LEN);
            {
                ...
                for (i = 0; i < count; i++)
                {
                    ...
                    banout_append(banout, PROTO_COAP, px + links[i].link_offset,
                                  links[i].link_length);
                }
                free(links);
            }
            break;
        case 41:
            banout_append(banout, PROTO_COAP, " application/xml", AUTO_LEN);
            break;
        case 42:
            banout_append(banout, PROTO_COAP, " application/octet-stream", AUTO_LEN);
            break;
        case 47:
            banout_append(banout, PROTO_COAP, " application/exi", AUTO_LEN);
            break;
        case 50:
            banout_append(banout, PROTO_COAP, " application/json", AUTO_LEN);
            break;
        default:
            banout_append(banout, PROTO_COAP, " (unknown-content-type)", AUTO_LEN);
            break;
    }
...
/***************************************************************************
 ***************************************************************************/
unsigned coap_handle_response(struct Output* out, time_t timestamp, const unsigned char* px,
                              unsigned length, struct PreprocessedInfo* parsed, uint64_t entropy)
{
    ...
    /* Validate the "syn-cookie" style information, which should match the
     * "Message ID field*/
    ...
    if (is_valid)
    {
        output_report_banner(out, timestamp, ip_them, 17 /*udp*/, parsed->port_src, PROTO_COAP,
                             parsed->ip_ttl, banout_string(banout, PROTO_COAP),
...
/****************************************************************************
 ****************************************************************************/
unsigned coap_udp_set_cookie(unsigned char* px, size_t length, uint64_t seqno)
{
    /*
 ...
 * given list.
 ****************************************************************************/
static int test_is_link(const char* name, const unsigned char* vinput, struct CoapLink* links,
                        size_t count, int line_number)
{
    size_t i;
...
int proto_coap_selftest(void)
{
    ...
    /* test quoted */
    {
        static const unsigned char* input =
            (const unsigned char*) "</sensors/temp>;if=\"se\\\"\\;\\,\\<\\>\\\\nsor\",</success>";
    ...
    /* test a simple link */
    {
        static const unsigned char* input = (const unsigned char*) "</sensors/temp>;if=\"sensor\"";
        links = parse_links(input, 0, (unsigned) (~0), &count);
    ...
    /* Test a complex dump */
    {
        ...
        (const unsigned char
             *)"</sensors/temp>;if=\"sensor\","
               ...
               "</sensors/temp>;rt=\"temperature-c\";if=\"sensor\","
               "</sensors/light>;rt=\"light-lux\";if=\"sensor\","
               "<http://www.example.com/sensors/t123>;anchor=\"/sensors/"
    ...
    /* Now test an entire packet */
    {
        ...
        /* parse a test packet */
        is_valid = coap_parse((const unsigned char*) input, sizeof(input) - 1, banout, &request_id);
        // fprintf(stderr, "[+] %.*s\n", (int)banout_string_length(banout,
        // PROTO_COAP), banout_string(banout, PROTO_COAP));
