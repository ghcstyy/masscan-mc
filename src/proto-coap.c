...
/****************************************************************************
 ****************************************************************************/
static struct CoapLink *parse_links(const unsigned char *px, unsigned offset,
                                    unsigned length, size_t *r_count) {
  struct CoapLink *l;
...
/****************************************************************************
 ****************************************************************************/
static bool coap_parse(const unsigned char *px, size_t length,
  ...
  *request_id = px[2] << 8 | px[3];
  ...
  {
    char buf[64];
    snprintf(buf, sizeof(buf), "rsp=%u.%u(%s)", code >> 5, code & 0x1F,
             response_code(code));
...
/***************************************************************************
 ***************************************************************************/
unsigned coap_handle_response(struct Output *out, time_t timestamp,
   ...
   * "Message ID field*/
  cookie = (unsigned)syn_cookie(ip_them, port_them | Templ_UDP, ip_me, port_me,
                                entropy);
  ...
  if (is_valid) {
    output_report_banner(out, timestamp, ip_them, 17 /*udp*/, parsed->port_src,
                         PROTO_COAP, parsed->ip_ttl,
 ...
 * given list.
 ****************************************************************************/
static int test_is_link(const char *name, const unsigned char *vinput,
                        struct CoapLink *links, size_t count, int line_number) {
  size_t i;
...
int proto_coap_selftest(void) {
  ...
  {
    static const unsigned char *input =
        (const unsigned char *)"</sensors/temp>;if=\"sensor\"";
    links = parse_links(input, 0, (unsigned)(~0), &count);
  ...
  /* Now test an entire packet */
  {
    ...
    /* parse a test packet */
    is_valid = coap_parse((const unsigned char *)input, sizeof(input) - 1,
                          banout, &request_id);
