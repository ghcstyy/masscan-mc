...
int pcap_setdirection(pcap_t* pcap, pcap_direction_t direction)
{
    ...
    if (real_setdirection == 0)
    {
        ...
        real_setdirection =
            (int (*)(pcap_t*, pcap_direction_t)) GetProcAddress(h, "pcap_setdirection");
        if (real_setdirection == 0)
        {
                        ...
                        const unsigned char** packet)
{
    ...
    again:
        err = PFRING.recv(adapter->ring, (unsigned char**) packet, 0, /* zero-copy */
                             ...
                             struct TemplateSet* tmplset)
{
     ...
     */
    template_set_target_ipv4(tmplset, ip_them, port_them, ip_me, port_me, seqno, px, sizeof(px),
                             &packet_length);
                             ...
                             struct TemplateSet* tmplset)
{
     ...
     * Construct the destination packet
     */
    template_set_target_ipv6(tmplset, ip_them, port_them, ip_me, port_me, seqno, px, sizeof(px),
                             &packet_length);
...
/***************************************************************************
 ***************************************************************************/
static void rawsock_close_adapter(struct Adapter* adapter)
{
    if (adapter->ring)
    {
...
static int is_pfring_dna(const char* name)
{
    ...
    if (!isdigit(name[0] & 0xFF))
        return 0;
    while (isdigit(name[0] & 0xFF)) name++;
                                     ...
                                     unsigned vlan_id)
{
    ...
    { /*First ensure pfring dna adapter is available*/
        fprintf(stderr,
                "No pfring adapter available. Please install pfring or run "
                "masscan without the --pfring option.\n");
        return 0;
    }
