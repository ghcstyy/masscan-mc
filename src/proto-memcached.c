...
/***************************************************************************
 ***************************************************************************/
static void memcached_tcp_parse(const struct Banner1* banner1, void* banner1_private,
    ...
    for (i = 0; i < length; i++)
    {
        switch (state)
        {
            case 0: /* command */
                ...
                /* drop through */
            case 1:
                id = smack_search_next(sm_memcached_responses, &memcached->match, px, &i,
                                       (unsigned) length);
                i--;
                switch (id)
                {
            ...
            case 200:
                ...
                break;
            case 101:
                id = smack_search_next(sm_memcached_stats, &memcached->match, px, &i,
                                       (unsigned) length);
                i--;
                switch (id)
                {
                    ...
                    case MS_TIME:
                    case MS_VERSION:
                        banout_append(banout, PROTO_MEMCACHED, memcached_stats[id].pattern,
                                      AUTO_LEN);
                        if (px[i] == '\n')
                            state = 0;
                        else
...
/***************************************************************************
 ***************************************************************************/
static void* memcached_init(struct Banner1* b)
{
    unsigned i;
    ...
    for (i = 0; memcached_responses[i].pattern; i++)
    {
        ...
        {
            tmp[len] = " \t\r\n"[j];
            smack_add_pattern(b->memcached_responses, tmp, (unsigned) len + 1,
                              memcached_responses[i].id, memcached_responses[i].is_anchored);
        }
    ...
    for (i = 0; memcached_stats[i].pattern; i++)
    {
        ...
        {
            tmp[len] = " \t\r\n"[j];
            smack_add_pattern(b->memcached_stats, tmp, (unsigned) len + 1, memcached_stats[i].id,
...
/***************************************************************************
 ***************************************************************************/
unsigned memcached_udp_parse(struct Output* out, time_t timestamp, const unsigned char* px,
                             unsigned length, struct PreprocessedInfo* parsed, uint64_t entropy)
{
    ...
    */
    ...
    /* Parse the remainder of the packet as if this were TCP */
    {
        ...
        memcached_tcp_parse(0, 0, stuff, px + 8, length - 8, banout, 0);
    }
    ...
    output_report_banner(out, timestamp, ip_them, 17 /*UDP*/, parsed->port_src, PROTO_MEMCACHED,
                         parsed->ip_ttl, banout_string(banout, PROTO_MEMCACHED),
...
/****************************************************************************
 ****************************************************************************/
unsigned memcached_udp_set_cookie(unsigned char* px, size_t length, uint64_t seqno)
{
    /*
    The frame header is 8 bytes long, as follows (all values are 16-bit integers
    in network byte order, high byte first):
...
/***************************************************************************
 ***************************************************************************/
static int memcached_selftest(void)
{
    return 0;
}
...
const struct ProtocolParserStream banner_memcached = {
    "memcached", 11211, "stats\r\n", 7, 0, memcached_selftest, memcached_init, memcached_tcp_parse,
};
