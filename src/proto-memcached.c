...
/***************************************************************************
 ***************************************************************************/
static void memcached_tcp_parse(const struct Banner1 *banner1,
...
static void *memcached_init(struct Banner1 *b) {
  ...
  b->memcached_responses =
      smack_create("memcached-responses", SMACK_CASE_INSENSITIVE);
  ...
  for (i = 0; memcached_stats[i].pattern; i++) {
    ...
    for (j = 0; j < 4; j++) {
      tmp[len] = " \t\r\n"[j];
      smack_add_pattern(b->memcached_stats, tmp, (unsigned)len + 1,
                        memcached_stats[i].id, memcached_stats[i].is_anchored);
    }
...
/***************************************************************************
 ***************************************************************************/
static int memcached_selftest(void) { return 0; }

/***************************************************************************
 ***************************************************************************/
const struct ProtocolParserStream banner_memcached = {
    "memcached",        11211,          "stats\r\n",         7, 0,
    memcached_selftest, memcached_init, memcached_tcp_parse,
};
