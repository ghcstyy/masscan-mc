...
void errmsg_init(unsigned long long in_entropy) { _entropy = in_entropy; }

static void _errmsg(const char *fmt, va_list marker) {
  fprintf(stderr, "[-] ERR: ");
  vfprintf(stderr, fmt, marker);
  fflush(stderr);
}

static void _errmsgip(ipaddress ip, unsigned port, const char *fmt,
                      va_list marker) {
