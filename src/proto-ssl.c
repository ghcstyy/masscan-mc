 ...
 */
#define DROPDOWN(i, length, state) \
    (state)++;                     \
    if (++(i) >= (length))         \
    break
...
/*****************************************************************************
 *****************************************************************************/
static void BANNER_CIPHER(struct BannerOutput* banout, unsigned cipher_suite)
{
    // const char *notes = "";
    char foo[64];
...
/*****************************************************************************
 *****************************************************************************/
static void BANNER_VERSION(struct BannerOutput* banout, unsigned version_major,
                           unsigned version_minor)
{
    ...
    switch (version_major << 8 | version_minor)
    {
        ...
        case 0x0304:
            ...
            break;
        default:
            snprintf(foo, sizeof(foo), "SSLver[%u,%u] ", version_major, version_minor);
 ...
 *****************************************************************************/
static void parse_server_hello(const struct Banner1* banner1, void* banner1_private,
                               ...
                               struct BannerOutput* banout, struct stack_handle_t* socket)
{
    ...
    */
    ...
    for (i = 0; i < length; i++) switch (state)
        {
            case VERSION_MAJOR:
 ...
 *****************************************************************************/
static void parse_server_cert(const struct Banner1* banner1, void* banner1_private,
                              struct StreamState* pstate, const unsigned char* px, size_t length,
                              struct BannerOutput* banout, struct stack_handle_t* socket)
{
    ...
    for (i = 0; i < length; i++) switch (state)
        {
            ...
            case CACERT:
            {
                ...
                if (banner1->is_capture_cert)
                {
                    banout_append_base64(banout, proto, px + i, len, &pstate->base64);
                }
                ...
                if (cert_remaining == 0)
                {
                    ...
                    if (banner1->is_capture_cert)
                    {
                        banout_finalize_base64(banout, proto, &pstate->base64);
 ...
 *****************************************************************************/
static void parse_handshake(const struct Banner1* banner1, void* banner1_private,
                            struct StreamState* pstate, const unsigned char* px, size_t length,
                            struct BannerOutput* banout, struct stack_handle_t* socket)
{
    ...
    for (i = 0; i < length; i++) switch (state)
        {
            ...
            case CONTENTS:
            {
                ...
                switch (ssl->handshake.type)
                {
                    ...
                    case 2: /* server hello */
                        parse_server_hello(banner1, banner1_private, pstate, px + i, len, banout,
                                           socket);
                        break;
                    case 11: /* server certificate */
                        parse_server_cert(banner1, banner1_private, pstate, px + i, len, banout,
                                          socket);
                        break;
                }
 ...
 *
 *****************************************************************************/
static void parse_heartbeat(const struct Banner1* banner1, void* banner1_private,
                            struct StreamState* pstate, const unsigned char* px, size_t length,
                            struct BannerOutput* banout, struct stack_handle_t* socket)
{
    struct SSLRECORD* ssl = &pstate->sub.ssl;
    ...
    for (i = 0; i < length; i++) switch (state)
        {
            ...
            case CONTENTS:
            {
                ...
                {
                    banout_append_base64(banout, PROTO_HEARTBLEED, px + i, len, &pstate->base64);
                }
 ...
 *
 *****************************************************************************/
static void parse_alert(const struct Banner1* banner1, void* banner1_private,
                        struct StreamState* pstate, const unsigned char* px, size_t length,
                        struct BannerOutput* banout, struct stack_handle_t* socket)
{
    struct SSLRECORD* ssl = &pstate->sub.ssl;
    ...
    for (i = 0; i < length; i++) switch (state)
        {
            ...
            case DESCRIPTION:
                    ...
                    switch (ssl->x.server_alert.description)
                    {
                        case 86:
                            if (!banout_is_contains(banout, PROTO_SAFE, "TLS_FALLBACK_SCSV"))
                                banout_append(banout, PROTO_SAFE, "poodle[TLS_FALLBACK_SCSV] ",
                                              AUTO_LEN);
                            break;
                        case 40:
                            if (!banout_is_contains(banout, PROTO_SAFE, "TLS_FALLBACK_SCSV"))
                                banout_append(banout, PROTO_SAFE, "poodle[no-SSLv3] ", AUTO_LEN);
                            break;
                        default:
                            banout_append(banout, PROTO_SAFE, "poodle[no-SSLv3] ", AUTO_LEN);
                            snprintf(foo, sizeof(foo), " ALERT(0x%02x%02x) ",
                                     ssl->x.server_alert.level, ssl->x.server_alert.description);
                ...
                {
                    char foo[64];
                    snprintf(foo, sizeof(foo), " ALERT(0x%02x%02x) ", ssl->x.server_alert.level,
 ...
 *
 *****************************************************************************/
static void ssl_parse_record(const struct Banner1* banner1, void* banner1_private,
                             struct StreamState* pstate, const unsigned char* px, size_t length,
                             struct BannerOutput* banout, struct stack_handle_t* socket)
{
            ...
            case CONTENTS:
            {
                ...
                switch (ssl->type)
                {
                    ...
                    case 21: /* alert */
                         ...
                         * be encrypted */
                        parse_alert(banner1, banner1_private, pstate, px + i, len, banout, socket);
                        break;
                    case 22: /* handshake */
                        parse_handshake(banner1, banner1_private, pstate, px + i, len, banout,
                                        socket);
                        break;
                    ...
                    case 24: /* heartbeat */
                        ...
                        parse_heartbeat(banner1, banner1_private, pstate, px + i, len, banout,
                                        socket);
                        break;
                }
...
static const char ssl_hello_template[] =
    ...
    "\x97\xe5\x60\x50\xc4\xa5\x4a\xe0\xb9\x01\x75\x15"
    "\x31\x23\x27\x68" /* random
                        */
    "\x87\xdc\x3d\x66\xec\x07\xdc\xa0\xe5\x1f\x1f\xa1"
    "\x3f\x49\xf8\xfc" /* TODO:
                          re-randomize
                          for
                          each
                          request,
    ...
    "\xc0\x2b\xcc\xa9\xc0\x2c\xc0\x09\xc0\x0a\xc0\x23"
    "\xc0\x24\xc0\x2f"
    "\xcc\xa8\xc0\x30\xc0\x13\xc0\x14\xc0\x27\xc0\x28"
    "\x00\x9e\xcc\xaa"
    "\x00\x9f\x00\x33\x00\x39\x00\x67\x00\x6b\x00\x9c"
    "\x00\x9d\x00\x3c"
    ...
    /* extensions */
    "\x00\x0b\x00\x04\x03\x00\x01\x02\x00\x0a\x00\x0c"
    "\x00\x0a\x00\x1d"
    "\x00\x17\x00\x1e\x00\x19\x00\x18\x00\x23\x00\x00"
    "\x00\x16\x00\x00"
    "\x00\x17\x00\x00\x00\x0d\x00\x30\x00\x2e\x04\x03"
    "\x05\x03\x06\x03"
    "\x08\x07\x08\x08\x08\x09\x08\x0a\x08\x0b\x08\x04"
    "\x08\x05\x08\x06"
    "\x04\x01\x05\x01\x06\x01\x03\x03\x02\x03\x03\x01"
    "\x02\x01\x03\x02"
 ...
 * select various options.
 *****************************************************************************/
static const char ssl_12_hello_template[] =
    "\x16\x03\x01\x01\x1a"
    "\x01"
    "\x00\x01\x16"
    "\x03\x03\x02\x58\x33\x79\x5f\x71\x03\xef\x07\xfe\x36\x61\xb0\x32\x81\xaa"
    "\x99\x10\x87\x6a\x8e\x5b\xf9\x03\x93\x44\x58\x4b\x19\xff\x42\x6a\x20\x64"
    "\x84\xcd\x28\x9c\xe9\xb1\x9d\xcd\x8a\x11\x4c\x3b\x40\x1c\x90\x02\xf2\xb5"
    "\x1a\xf1\x7e\x5d\xb8\x42\xc2\x1e\x17\x1e\x59\xa4\xac\x00\x3e\x13\x02\x13"
    "\x03\x13\x01\xc0\x2c\xc0\x30\x00\x9f\xcc\xa9\xcc\xa8\xcc\xaa\xc0\x2b\xc0"
    "\x2f\x00\x9e\xc0\x24\xc0\x28\x00\x6b\xc0\x23\xc0\x27\x00\x67\xc0\x0a\xc0"
    "\x14\x00\x39\xc0\x09\xc0\x13\x00\x33\x00\x9d\x00\x9c\x00\x3d\x00\x3c\x00"
    "\x35\x00\x2f\x00\xff\x01\x00\x00\x8f\x00\x0b\x00\x04\x03\x00\x01\x02\x00"
    "\x0a\x00\x0c\x00\x0a\x00\x1d\x00\x17\x00\x1e\x00\x19\x00\x18\x00\x23\x00"
    "\x00\x00\x16\x00\x00\x00\x17\x00\x00\x00\x0d\x00\x2a\x00\x28\x04\x03\x05"
    "\x03\x06\x03\x08\x07\x08\x08\x08\x09\x08\x0a\x08\x0b\x08\x04\x08\x05\x08"
    "\x06\x04\x01\x05\x01\x06\x01\x03\x03\x03\x01\x03\x02\x04\x02\x05\x02\x06"
    "\x02\x00\x2b\x00\x09\x08\x03\x04\x03\x03\x03\x02\x03\x01\x00\x2d\x00\x02"
    "\x01\x01\x00\x33\x00\x26\x00\x24\x00\x1d\x00\x20\xb6\x87\xb7\x72\xb9\xcb"
    "\x07\xe0\x14\x0a\x14\x81\x3f\x3f\x0a\xcc\xc4\x7d\x80\xf7\xe8\xaa\x1e\x73"
    "\xb0\xa9\xad\xb8\x3a\xa7\x3c\x64";
;
/*****************************************************************************
 *****************************************************************************/
static char* ssl_add_cipherspec_sslv3(void* templ, unsigned cipher_spec, unsigned is_append)
{
    unsigned char* px;
    ...
    if (is_append)
    {
        ...
        memmove(px + offset2 + len2 + 2, px + offset2 + len2, len0 - (offset2 + len2));
    ...
    else
    {
        ...
        memmove(px + offset2 + 2, px + offset2, len0 - offset2);
...
static int ssl_selftest(void)
{
     ...
     */
    {
        ...
        x = banout_is_equal(banout1, PROTO_SSL3, ", www.google.com, www.google.com");
        if (!x)
        {
            printf("x.509 parser failure: google.com\n");
            return 1;
    ...
    banout_init(banout1);
    {
        ...
        for (i = 0; i < ssl_test_case_3_size; i++)
            ssl_parse_record(banner1, 0, state, ssl_test_case_3 + i, 1, banout1, 0);
    }
    /*if (0) {
    ...
    for (ii = 0; ii < ssl_test_case_3_size; ii++)
        ssl_parse_record(banner1, 0, state, (const unsigned char*) ssl_test_case_3 + ii, 1, banout2,
                         0);
    banner1_destroy(banner1);
...
struct ProtocolParserStream banner_ssl_12 = {
    ...
    ssl_selftest, ssl_init, ssl_parse_record,
};
